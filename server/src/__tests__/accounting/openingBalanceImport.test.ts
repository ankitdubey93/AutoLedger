...
/**
 * Accounting — the staged opening-balance importer (Phase 9b). Integration
...
it('a row on the receivable control account is refused', async () => {
  ...
  expect(row.errors.some((e) => e.includes('through /invoices'))).toBe(true);
});
...
it('a row on the payable control account is refused', async () => {
  ...
  expect(row.errors.some((e) => e.includes('through /bills'))).toBe(true);
});
