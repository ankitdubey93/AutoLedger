...
/**
 * Phase 32 — a Accounting bill receives stock and a Accounting invoice issues
 * it, posting to the GL through Inventory's document seam. Integration tier,
...
describe('a Inventory item is a Accounting product', () => {
  ...
  it('Accounting refuses to create an INVENTORY item directly', async () => {
...
describe('manual Inventory movements of a linked item post to the GL', () => {
...
describe('linking a pre-existing stock item', () => {
  ...
  it('renaming or deactivating in Inventory syncs the linked product', async () => {
    const stocked = await createStockedItem(agentA, orgA);
  ...
  it('Accounting refuses to rename a stock-managed product', async () => {
    const stocked = await createStockedItem(agentA, orgA);
