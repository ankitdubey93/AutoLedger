...
/**
 * Accounting — the staged customer/vendor importer (Phase 24). Integration
...
it('committing creates the customers', async () => {
  ...
  const list = await agentA.get('/api/v1/customers');
...
it('a name already in the system merges instead of duplicating', async () => {
  await agentA.post('/api/v1/customers').send({ name: 'Harbor Point' });
...
it('a merge never overwrites an existing value', async () => {
  await agentA.post('/api/v1/customers').send({ name: 'Harbor Point', email: 'old@example.com' });
...
it('a vendor CSV maps payment terms onto the vendor', async () => {
  ...
  const list = await agentA.get('/api/v1/vendors');
