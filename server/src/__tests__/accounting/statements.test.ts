...
/**
 * Accounting — Profit & Loss and the balance sheet (Phase 4). Integration
...
const PROFIT_AND_LOSS = '/api/v1/reports/profit-and-loss';
const BALANCE_SHEET = '/api/v1/reports/balance-sheet';
...
describe('GET /reports/balance-sheet', () => {
  ...
  it('confirms no summary table exists anywhere in the schema', async () => {
    ...
    // deliberate pattern — a derived CACHE for Inventory's perpetual
    ...
    // not a stored summary Accounting's own reports could silently drift
    // from. This test's own intent (its describe block, its header comment
    // above) is that Accounting's balance sheet has no such table — it
