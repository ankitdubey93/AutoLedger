...
import { trialBalance } from '../../services/accounting/reportService.js';
import { createUserWithOrg, loginAgent, resetTables } from '../helpers/factories.js';
import type { SeededUser } from '../helpers/factories.js';

/**
 * Accounting — the trial balance. Integration tier.
 *
...
const TRIAL_BALANCE = '/api/v1/reports/trial-balance';
...
describe('trial balance', () => {
  ...
  it('?asOf excludes entries dated after the cutoff', async () => {
    ...
    expect(july.body.count).toBe(37);
  });
  ...
  it('excludes a retired account once it has been deactivated', async () => {
    ...
    await agent.patch(`/api/v1/accounts/${id}`).send({ isActive: false });
