...
/**
 * Inventory (Phase 28) — QR label generation and scan-and-lookup.
...
const LABELS = '/api/v1/inventory/labels';
const LOOKUP = '/api/v1/inventory/lookup';
...
describe('POST /inventory/labels', () => {
  it('builds an ITEM label with an SVG QR and a scan URL payload', async () => {
    ...
    expect(label.payload).toBe(`${env.FRONTEND_URL.replace(/\/+$/, '')}/inventory/scan/item/${f.cmpItemId}`);
