...
/**
 * LedgerCore — payment terms (Phase 24). Integration tier, real PostgreSQL.
 *
...
const app = createApp();
const TERMS_BASE = '/api/v1/ledger-core/payment-terms';
...
async function revenueAccountId(agent: import('supertest').Agent): Promise<string> {
  const res = await agent.get('/api/v1/ledger-core/accounts');
...
async function vendorAndExpenseAccount(
  ...
  const vendorRes = await agent.post('/api/v1/ledger-core/vendors').send({ name: 'Acme Supplies' });
  const accountsRes = await agent.get('/api/v1/ledger-core/accounts');
...
describe('GET /ledger-core/payment-terms', () => {
...
describe('POST /ledger-core/payment-terms', () => {
...
describe('PATCH /ledger-core/payment-terms/:id', () => {
