...
describe('capture documents API', () => {
  ...
  it('registers a PDF vault document and returns 201 with status PENDING', async () => {
    ...
    const agent = await loginAgent(app, userA);
    const res = await agent.post(CAPTURE_BASE).send({ documentId });
  ...
  it('registering the same vault document twice creates a second, DUPLICATE row rather than 409ing', async () => {
    const documentId = await uploadPdfAsA();
    const agent = await loginAgent(app, userA);
    const first = await agent.post(CAPTURE_BASE).send({ documentId });
    expect(first.status).toBe(201);
    expect(first.body.document.status).toBe('PENDING');

    const second = await agent.post(CAPTURE_BASE).send({ documentId });
  ...
  it('refuses a CSV vault document with 422', async () => {
    ...
    const res = await agent.post(CAPTURE_BASE).send({ documentId: upload.body.document.id });
    expect(res.status).toBe(422);
  });

  it('returns 404 for a syntactically valid but unknown document id', async () => {
    const agent = await loginAgent(app, userA);
    const res = await agent.post(CAPTURE_BASE).send({ documentId: randomUUID() });
    expect(res.status).toBe(404);
  });

  it('returns 400 for a non-UUID documentId', async () => {
    const agent = await loginAgent(app, userA);
    const res = await agent.post(CAPTURE_BASE).send({ documentId: 'not-a-uuid' });
  ...
  it('cross-tenant: org B cannot GET org A document by id (404, not 403)', async () => {
    ...
    const created = await agentA.post(CAPTURE_BASE).send({ documentId });
    const captureDocId = created.body.document.id as string;

    const agentB = await loginAgent(app, userB);
    await switchTo(agentB, orgB);
    const res = await agentB.get(`${CAPTURE_BASE}/${captureDocId}`);
  ...
  it('cross-tenant: org B cannot register a documentId belonging to org A (404)', async () => {
    ...
    const agentB = await loginAgent(app, userB);
    await switchTo(agentB, orgB);
    const res = await agentB.post(CAPTURE_BASE).send({ documentId });
  ...
  it('cross-tenant: org B list does not contain org A document', async () => {
    const documentId = await uploadPdfAsA();
    const agentA = await loginAgent(app, userA);
    await agentA.post(CAPTURE_BASE).send({ documentId });

    const agentB = await loginAgent(app, userB);
    await switchTo(agentB, orgB);
    const res = await agentB.get(CAPTURE_BASE);
  ...
  it('cross-tenant: org B cannot fetch a page image for an org A document', async () => {
    ...
    const agentB = await loginAgent(app, userB);
    await switchTo(agentB, orgB);
    const res = await agentB.get(`${CAPTURE_BASE}/${captureDocId}/pages/1/image`);
  ...
  it('VIEWER can list but cannot register a document', async () => {
    ...
    const listRes = await agentViewer.get(CAPTURE_BASE);
    expect(listRes.status).toBe(200);

    const createRes = await agentViewer.post(CAPTURE_BASE).send({ documentId });
  ...
  it('ACCOUNTANT can register a document', async () => {
    ...
    const res = await agentAccountant.post(CAPTURE_BASE).send({ documentId });

    expect(res.status).toBe(201);
  });

  it('unauthenticated request is rejected with 401', async () => {
    const res = await request(app).get(CAPTURE_BASE);
  ...
  it('re-extract on a PENDING document returns 409 (PENDING -> PENDING is not an edge)', async () => {
    ...
    const res = await agentA.post(`${CAPTURE_BASE}/${captureDocId}/reextract`);
    expect(res.status).toBe(409);
  });

  it('GET /capture/documents/:id never returns an ocrText key', async () => {
    const documentId = await uploadPdfAsA();
    const agentA = await loginAgent(app, userA);
    const created = await agentA.post(CAPTURE_BASE).send({ documentId });
    const captureDocId = created.body.document.id as string;

    const res = await agentA.get(`${CAPTURE_BASE}/${captureDocId}`);
    expect(res.status).toBe(200);
