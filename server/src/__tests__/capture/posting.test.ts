 ...
 * to post as a real Accounting bill rather than a raw journal entry — see
...
): Promise<{ captureDocId: string; sha256: string }> {
  ...
  for (const [index, item] of options.lineItems.entries()) {
    await pool.query(
      `INSERT INTO ap_flow_line_items (org_id, ap_flow_document_id, line_index, description, amount_cents, account_id)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [orgId, captureDocId, index, item.description, item.amountCents, item.accountId],
    );
  }

  return { captureDocId, sha256 };
}

describe('capture posting API', () => {
  beforeEach(async () => {
    await resetTables();
    await clearStorage();
  ...
  it('a two-line receipt posts one balanced entry debiting two different accounts', async () => {
    ...
    const { captureDocId } = await seedExtractedDocument(orgA, userA.id, {
      subtotalCents: 20000,
      taxCents: 0,
      totalCents: 20000,
      lineItems: [
    ...
    const res = await agent.post(`${CAPTURE_BASE}/${captureDocId}/post`);
  ...
  it('re-posting the same document does not create a second entry', async () => {
    ...
    const agent = await loginAgent(app, userA);
    const first = await agent.post(`${CAPTURE_BASE}/${captureDocId}/post`);
    expect(first.status).toBe(200);
    const billId = first.body.document.billId as string;

    const second = await agent.post(`${CAPTURE_BASE}/${captureDocId}/post`);
  ...
  it("the posted bill resolves back to the document's stored hash", async () => {
    const account = await accountIdByCode(orgA, '6130');
    const { captureDocId, sha256 } = await seedExtractedDocument(orgA, userA.id, {
      subtotalCents: 5000,
      totalCents: 5000,
      lineItems: [{ description: 'Office supplies', amountCents: 5000, accountId: account }],
    });
    ...
    const getRes = await agent.get(`${CAPTURE_BASE}/${captureDocId}`);
  ...
  it('a foreign-currency document uses the rate at its invoice date', async () => {
    ...
    const res = await agentSetup.post(`${CAPTURE_BASE}/${captureDocId}/post`);
    expect(res.status).toBe(200);
  ...
  it('posting refuses a document with no line items', async () => {
    const { captureDocId } = await seedExtractedDocument(orgA, userA.id, { lineItems: [] });
  ...
  it('posting records the vendor in the account map', async () => {
    ...
    const agent = await loginAgent(app, userA);
    const res1 = await agent.post(`${CAPTURE_BASE}/${first.captureDocId}/post`);
    ...
    const res2 = await agent.post(`${CAPTURE_BASE}/${second.captureDocId}/post`);
  ...
  it('posting another organization\'s document returns 404', async () => {
    ...
    const { captureDocId } = await seedExtractedDocument(orgB, (await createUserWithOrg({})).id, {
      subtotalCents: 1000,
      totalCents: 1000,
  ...
  it('posting creates a POSTED bill and AP aging reconciles', async () => {
    ...
    const agingRes = await agent.get('/api/v1/reports/ap-aging');
  ...
  it('posting a duplicate invoice number for the same vendor returns 409 and leaves the second document EXTRACTED', async () => {
    ...
    const agent = await loginAgent(app, userA);
    const firstRes = await agent.post(`${CAPTURE_BASE}/${first.captureDocId}/post`);
    expect(firstRes.status).toBe(200);

    const secondRes = await agent.post(`${CAPTURE_BASE}/${second.captureDocId}/post`);
    expect(secondRes.status).toBe(409);
    expect(secondRes.body.error).toContain('already exists');

    const getRes = await agent.get(`${CAPTURE_BASE}/${second.captureDocId}`);
    expect(getRes.body.document.status).toBe('EXTRACTED');
