...
const REVIEW_QUEUE = '/api/v1/capture/review-queue';
...
describe('capture review queue and line-item override API', () => {
  ...
  async function seedExtractedDocument(
    ...
    await pool.query(
      ...
      [orgId, captureDocId, options.arithmeticOk ?? true, JSON.stringify(fieldConfidence)],
    );
  ...
  it('sets the account and marks the source MANUAL', async () => {
    ...
    const agent = await loginAgent(app, userA);
    const getRes = await agent.get(`${CAPTURE_BASE}/${docId}`);
    const lineItemId = getRes.body.document.lineItems[0].id as string;

    const res = await agent
      .patch(`${CAPTURE_BASE}/${docId}/line-items/${lineItemId}`)
  ...
  it('rejects a line item belonging to another document with 404', async () => {
    ...
    const agent = await loginAgent(app, userA);
    const otherDoc = await agent.get(`${CAPTURE_BASE}/${otherDocId}`);
    const otherLineItemId = otherDoc.body.document.lineItems[0].id as string;

    const res = await agent
      .patch(`${CAPTURE_BASE}/${docId}/line-items/${otherLineItemId}`)
      .send({ accountId: newAccountId });
  ...
  it("returns 404 for another organization's document", async () => {
    ...
    const res = await agent
      .patch(`${CAPTURE_BASE}/${docId}/line-items/00000000-0000-0000-0000-000000000000`)
      .send({ accountId: newAccountId });

    expect(res.status).toBe(404);
    expect(res.body.error).toContain('Captured document not found');
  });
  ...
  it('is refused for a VIEWER with 403', async () => {
    ...
    const getRes = await getAsA.get(`${CAPTURE_BASE}/${docId}`);
    const lineItemId = getRes.body.document.lineItems[0].id as string;
