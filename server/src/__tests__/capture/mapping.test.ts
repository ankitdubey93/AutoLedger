...
} from '../../services/capture/mappingService.js';
import type { ClassificationClient } from '../../services/capture/mappingService.js';
import type { OnModelCall } from '../../services/capture/extractionService.js';
...
describe('mappingService.classifyLineItems', () => {
  ...
  it('a model classification reports one CLASSIFY call carrying the document id', async () => {
    ...
    await classifyLineItems(
      orgA,
      { vendorName: 'Some New Vendor', lineItems: [{ description: 'zzzzzzzz qqqq', amountCents: 1000 }] },
      { classifier, onModelCall, captureDocumentId: 'doc-123' },
    );
  ...
  it('saveLineItemsOnClient replaces prior rows', async () => {
    ...
    await withTransaction((client) =>
      saveLineItemsOnClient(client, orgA, captureDocId, [
