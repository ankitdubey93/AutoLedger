...
import type { StockLabel, StockLabelKind } from '../../types/stock.js';

/**
 * StockLedger (Phase 28) — QR label generation.
 *
 * The QR payload is `${FRONTEND_URL}/app/stock/scan/<kind>/<id>` — a route
 * and a UUID, nothing else. No org id, no name, no price is ever encoded:
...
export async function buildLabels(orgId: string, targets: LabelTarget[]): Promise<StockLabel[]> {
  ...
  for (const target of targets) {
    ...
    const payload = `${base}/app/stock/scan/${target.kind.toLowerCase()}/${target.id}`;
