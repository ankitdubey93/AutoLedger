...
/**
 * StockLedger's document seam (Phase 32) — the ONE module LedgerCore's
 * invoice and bill services import from StockLedger (guardrails rule 16).
 *
 * A bill line for an INVENTORY product receives stock; an invoice line issues
 * it. LedgerCore owns the documents and builds its own journal entry; this
 * file owns the stock: it moves quantity, values the movement, and hands the
 * values back so LedgerCore can put them in the entry it posts. Nothing here
...
export interface DocumentStockLine {
  lineNumber: number;
  /** The LedgerCore product (items.id) the line was picked from. */
  ledgerItemId: string;
  /** Null means "the default location" (stock_settings.default_location_id). */
  locationId: string | null;
  quantityMilli: number;
  /** The inventory account this line's value posts to (resolved by LedgerCore). */
  glAccountId: string;
...
async function loadLinkedItems(
  ...
  // 500), not user input — StockLedger creates both in one transaction.
...
async function resolveLines(
  ...
  for (const line of lines) {
    ...
    if (stockItem.tracking !== 'QUANTITY') {
      throw new ApiError(
        422,
        `Line ${String(line.lineNumber)}: item ${stockItem.code} is ${stockItem.tracking.toLowerCase()}-tracked — lot/serial selection on invoice and bill lines is not supported yet; record it in StockLedger`,
      );
    }
