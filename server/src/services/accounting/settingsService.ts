...
/**
 * Accounting onboarding and settings.
 *
...
/** GET /settings. A missing `ledger_settings` row is not a 404 — it means "not yet onboarded". */
...
/**
 * Completes (or re-completes) Accounting onboarding for one organization.
 *
...
export async function completeOnboarding(orgId: string, input: OnboardingInput): Promise<LedgerSettings> {
  ...
  try {
    ...
    // Marks Accounting's own onboarding row COMPLETED on this same client, so
    // it commits or rolls back together with the settings write (rule 5).
    await onboardingService.markCompletedOnClient(client, orgId, MODULE_TAGS.accounting);
 ...
 * and Capture's `postingService.postCaptureDocument` both need it, and this
