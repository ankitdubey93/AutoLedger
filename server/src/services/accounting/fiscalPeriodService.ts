...
/**
 * Accounting fiscal periods — Phase 4's close/lock lifecycle.
 *
...
export async function generatePeriods(
  ...
  try {
    ...
    if (settings === undefined) {
      throw new ApiError(409, 'Complete setup before generating fiscal periods');
    }
...
async function transition(
  ...
  try {
    ...
    if (to === 'CLOSED') {
      ...
      // system acts on.
      await emitEvent(client, orgId, MODULE_TAGS.accounting, 'fiscal_period.closed', {
        periodId: id,
        fiscalYearLabel: row.fiscal_year_label,
        periodNumber: row.period_number,
        startsOn: row.starts_on,
