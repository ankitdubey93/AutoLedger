...
import { AP_FLOW_AUTO_POST_DEFAULTS } from '../../types/ap-flow.js';
import type { ApFlowAutoPostBlocker, ApFlowSettings } from '../../types/ap-flow.js';

/**
 * AP-Flow's auto-post settings and the straight-through attempt itself
...
export async function getSettings(orgId: string): Promise<ApFlowSettings> {
  ...
  const row = rows[0];
  if (row === undefined) return { ...AP_FLOW_AUTO_POST_DEFAULTS };
...
export async function attemptAutoPost(orgId: string, apFlowDocumentId: string): Promise<'POSTED' | 'BLOCKED' | 'SKIPPED'> {
  ...
  let doc;
  try {
    doc = await apFlowDocumentService.getApFlowDocumentById(orgId, apFlowDocumentId);
  } catch (err) {
  ...
  if (blockers.length > 0) {
    await recordBlockersSafely(orgId, apFlowDocumentId, blockers);
    return 'BLOCKED';
  }

  try {
    await postingService.postApFlowDocument(orgId, doc.createdBy, apFlowDocumentId, { autoPosted: true });
    return 'POSTED';
  } catch (err) {
    const rejection: ApFlowAutoPostBlocker[] =
    ...
    if (!(err instanceof ApiError && (err.status === 409 || err.status === 422))) {
      console.error('[ap-flow] auto-post failed unexpectedly:', err);
    }

    await recordBlockersSafely(orgId, apFlowDocumentId, rejection);
    return 'BLOCKED';
  }
}

/** Recording a blocker must never itself fail the pipeline — log and move on. */
async function recordBlockersSafely(orgId: string, id: string, blockers: ApFlowAutoPostBlocker[]): Promise<void> {
  try {
    await apFlowDocumentService.recordAutoPostBlockers(orgId, id, blockers);
  } catch (err) {
    console.error('[ap-flow] failed to record auto-post blockers:', err);
  }
}
