...
import type { JournalLineInput } from '../accounting/journalService.js';
import { resolveInventoryPostingAccountsOnClient } from '../accounting/settingsService.js';
import type { StockMovement } from '../../types/inventory.js';

/**
 * Inventory -> Accounting general-ledger bridge for MANUAL movements
 * (Phase 32). Document-driven movements (a bill's receipt, an invoice's issue)
 * do not come through here — Accounting builds those journals itself out of the
 * values `documentStockService` returns.
 *
 * Rule 16: this file reaches Accounting only through its public service
 * functions (`itemService`, `settingsService`, `journalService`) and reads only
 * Inventory's own table (`stock_items`). Every function runs on the caller's
 ...
 * the GL inventory account starts equal to Inventory's valuation.
 */
