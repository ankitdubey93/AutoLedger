...
import { CAPTURE_CLASSIFY_MAX_TOKENS, CAPTURE_CLASSIFY_TIMEOUT_MS } from '../../config/constants.js';
...
import { classificationToolInputSchema } from '../../schemas/capture/mappingSchema.js';
import type { CaptureLineItem, CaptureMappingSource } from '../../types/capture.js';
import type { Account } from '../../types/accounting.js';
import {
...
/**
 * Capture's account classification (Phase 11) — GL coding inferred in a
 ...
 *
 * This file reads Accounting's chart of accounts exclusively through
 * `accountService`'s exported functions — never a query against `accounts`
 * — because Capture does not own that table (guardrails rule 16).
 */
...
export async function classifyLineItems(
  orgId: string,
  input: { vendorName: string | null; lineItems: CaptureLineItem[] },
  deps?: {
  ...
  const classifications: LineItemClassification[] = input.lineItems.map((item, lineIndex) => {
    ...
    if (bestAccountId !== null && bestScore >= CHART_MATCH_MIN_SIMILARITY) {
      return {
        ...
        mappingSource: 'CHART' as CaptureMappingSource,
    ...
    return {
      ...
      mappingSource: 'NONE' as CaptureMappingSource,
...
async function classifyWithModel(
  ...
  if (deps?.modelClient !== undefined) {
    ...
    console.warn('[capture] account classification unavailable: no AI provider key is configured');
    return results;
  }

  const entityType = deps?.captureDocumentId === undefined ? null : 'ap_flow_document';
  const entityId = deps?.captureDocumentId ?? null;
  ...
  try {
    ...
    const raw = await effectiveClient.generateStructured({
      ...
      maxTokens: CAPTURE_CLASSIFY_MAX_TOKENS,
      timeoutMs: CAPTURE_CLASSIFY_TIMEOUT_MS,
    });

    deps?.onModelCall?.({
      module: MODULE_TAGS.capture,
      purpose: 'CLASSIFY',
    ...
    console.warn(`[capture] account classification unavailable: ${message}`);
  }
 ...
 * mirroring `captureDocumentService.savePipelineResult`'s treatment of pages
