import { CAPTURE_VISION_MAX_TOKENS, CAPTURE_VISION_TIMEOUT_MS } from '../../config/constants.js';
import { ApiError } from '../../utils/apiError.js';
import { parseMoneyText } from '../../utils/money.js';
import { extractionToolInputSchema } from '../../schemas/capture/extractionSchema.js';
import type { CaptureLineItem } from '../../types/capture.js';
...
/**
 * Capture's vision extraction (Phase 10, multi-provider since Phase 19).
 ...
 *   Anthropic > the configured provider (env.CAPTURE_AI_PROVIDER).
 */
export async function extractFromPages(
  ...
  const request = {
    ...
    maxTokens: CAPTURE_VISION_MAX_TOKENS,
    timeoutMs: CAPTURE_VISION_TIMEOUT_MS,
  };
  ...
  const lineItems: CaptureLineItem[] = parsed.line_items.map((item) => ({
