...
/**
 * Capture (Phase 10) — capture & extraction. This file mirrors
 * `types/accounting.ts`'s FSM-table shape: one transition table in code,
 * a matching CHECK constraint in the migration.
 */

export const CAPTURE_DOCUMENT_STATUSES = ['PENDING', 'PROCESSING', 'EXTRACTED', 'FAILED', 'POSTED', 'DUPLICATE'] as const;
export type CaptureDocumentStatus = (typeof CAPTURE_DOCUMENT_STATUSES)[number];

export function isCaptureDocumentStatus(value: string): value is CaptureDocumentStatus {
  return (CAPTURE_DOCUMENT_STATUSES as readonly string[]).includes(value);
}
 ...
 * reversing its journal entry in Accounting, never by re-extracting or
 ...
 */
export const CAPTURE_DOCUMENT_TRANSITIONS = {
  PENDING: ['PROCESSING'],
  ...
  POSTED: [],
  DUPLICATE: ['PENDING'],
} as const satisfies Record<CaptureDocumentStatus, readonly CaptureDocumentStatus[]>;

export function canTransitionCaptureDocument(
  from: CaptureDocumentStatus,
  to: CaptureDocumentStatus,
): boolean {
  return (CAPTURE_DOCUMENT_TRANSITIONS[from] as readonly CaptureDocumentStatus[]).includes(to);
}
 ...
 */
export const CAPTURE_MAPPING_SOURCES = ['HISTORY', 'CHART', 'MODEL', 'MANUAL', 'NONE'] as const;
export type CaptureMappingSource = (typeof CAPTURE_MAPPING_SOURCES)[number];

export function isCaptureMappingSource(value: string): value is CaptureMappingSource {
  return (CAPTURE_MAPPING_SOURCES as readonly string[]).includes(value);
}
...
/** One row in GET /capture/review-queue, lowest confidence first. */
...
export interface CaptureDocumentRecord {
  id: string;
  documentId: string;
  originalFilename: string;
  mimeType: string;
  ...
  /** Phase 19. The Accounting bill this document posted as. No REFERENCES — rule 16. */
  billId: string | null;
...
export interface CaptureDocumentDetail extends CaptureDocumentRecord {
  pages: CapturePage[];
...
/**
 * GET /capture/documents filters, read with utils/queryParam.ts's readers —
 * the established pattern for list-endpoint query strings, not a zod schema.
 */
export interface CaptureDocumentListFilters {
  status: CaptureDocumentStatus | null;
  page: number;
  limit: number;
}
 ...
 */
export const CAPTURE_AUTO_POST_BLOCKER_CODES = [
  'AUTO_POST_DISABLED',
  'ARITHMETIC_MISMATCH',
  'MISSING_VENDOR_NAME',
...
export type CaptureAutoPostBlockerCode = (typeof CAPTURE_AUTO_POST_BLOCKER_CODES)[number];

export interface CaptureAutoPostBlocker {
  code: CaptureAutoPostBlockerCode;
  message: string;
}
...
export const CAPTURE_AUTO_POST_DEFAULTS: CaptureSettings = {
...
// integration is platform-level now, not Capture's, because a folder's purpose
