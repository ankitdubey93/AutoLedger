/**
 * Accounting's domain types — the general ledger.
 *
 * App-scoped, so this file is `types/accounting.ts` rather than sitting at the
...
/**
 * Accounting's onboarding and settings. `organizationName` and `baseCurrency`
...
export interface InvoiceLine {
  ...
  itemId: string | null;
  /** Phase 32 — the Inventory location an INVENTORY line moves stock at; null = the default location. */
...
/** Phase 32. SERVICE/NON_INVENTORY are created in Accounting; INVENTORY/FIXED_ASSET are created in Inventory. */
...
export interface BalanceSheetEquity extends StatementSection {
   ...
   * start — DERIVED on every read, never stored. Accounting posts no
...
/**
 * The three GL control accounts, resolved through Accounting so nothing
