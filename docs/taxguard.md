...
5. Embed every chunk through the injectable `EmbeddingsClient` seam, writing each vector back individually. Batches are capped at **64 inputs and 8,000 estimated tokens** (`TAXGUARD_EMBEDDING_MAX_BATCH_TOKENS`, measured against `estimateTokens`, which runs ~10% above Voyage's real count). The token cap exists because a Voyage account with no payment method on file is limited to **3 requests and 10K tokens per minute**, and a single request over 10K tokens is refused with `429` however long you wait. That was a real failure: a 50-page act's 64 chunks (~75K tokens) went out as one request and failed every time. A `429`, a `5xx` or a network failure is retried up to 6 times per batch with backoff of 15 s × 2ⁿ, capped at 60 s (Voyage sends no `Retry-After`). Only ingestion opts in (`embedTexts(…, { maxRetries })`). A question-time query embedding runs inside an HTTP request, so it keeps the default of 0 retries and fails fast. On the free tier, a 75K-token act takes about 10 minutes to embed. With a payment method on file, the same batches go through back to back. A provider failure's message carries the HTTP status and Voyage's own `detail` text (e.g. `Embeddings provider rate limit exceeded (HTTP 429): …`), because it becomes the row's `error_message`.
6. Advance `EMBEDDING → READY` with the final `chunk_count`.

A failure at any step calls `markFailed` and rethrows, so a terminal failure still reaches the dead-letter queue via the worker's existing `failed` listener. A BullMQ retry of the job is a no-op: the row is already `FAILED`, so the `PENDING → PARSING` guard returns early. Transient provider failures are therefore retried *inside* step 5, not by BullMQ. **`READY` and `FAILED` are both terminal** in `TAXGUARD_CORPUS_TRANSITIONS` — there is no in-place re-ingest. Re-adding the same `documentId` after a failed attempt is refused with `409` (`ux_taxguard_corpus_document`); the failed row must be deleted first.
...
**The embeddings provider is env-selected: `TAXGUARD_EMBEDDING_PROVIDER` = `voyage` (default; Voyage AI `voyage-3.5`) or `gemini` (`gemini-embedding-001` on the same `GEMINI_API_KEY` AP-Flow uses). Both return 1024 dimensions and are called over the platform's built-in `fetch`, not an installed SDK.** Anthropic ships no embeddings endpoint, and Voyage is its own documented recommendation. Calling either provider over `fetch` means **no new npm dependency** for embeddings at all. `embeddingService.ts` exports an injectable `EmbeddingsClient` seam exactly as `extractionService.ts`'s `VisionClient` does, so no test ever reaches the network. The two concrete clients are `voyageEmbeddingsClient` and `geminiEmbeddingsClient`, and `realEmbeddingsClient()` picks one from the env. Only the selected provider's key is needed. It is optional, matching `ANTHROPIC_API_KEY`'s own posture: the server and worker both boot without it, and ingestion/answering return `503` only when actually attempted with no key configured.

**Why Gemini was added (2026-09-22).** On an account with no payment method, Voyage allows 3 requests and 10K tokens per minute. A 50-page act (64 chunks, ~75K tokens) took ~16 minutes even after the batching fix in step 5 above. Gemini's free tier embedded the same document in under a minute, also through a couple of 429s that the ingestion retry absorbed. Details of the Gemini call:
- `batchEmbedContents`, one request entry per text, with `outputDimensionality: 1024`.
- `taskType` is `RETRIEVAL_DOCUMENT` for chunks and `RETRIEVAL_QUERY` for questions: the same asymmetric document/query split Voyage's `input_type` gives.
- The model's native output is 3072 dimensions; the 1024 prefix is a valid embedding because the model is Matryoshka-trained. It is not unit-length, and that doesn't matter because retrieval ranks by cosine distance (`<=>`), which ignores length.
- Gemini's input limit is 2,048 tokens per text. `taxActParse` targets 500 but emits an over-long single paragraph whole, so a few chunks of a poorly paragraphed PDF can exceed it. The live test shows no error for those; the tail past the limit is most likely left out of that chunk's vector (the chunk's full text is still stored and cited).
- Free-tier Gemini API content may be used by Google to improve its models. The question is already redacted before any provider call (§ D); a public tax act's chunks are not sensitive.

**Vectors from different providers are not comparable.** No column records which provider embedded a chunk. After switching `TAXGUARD_EMBEDDING_PROVIDER`, delete and re-add every corpus document, or retrieval will compare a Gemini query vector against Voyage chunk vectors and return noise. The same 0.25 `TAXGUARD_RETRIEVAL_MIN_SCORE` floor applies to both. It was not re-tuned for Gemini, whose cosine scores for unrelated text tend to run higher, so the floor filters less there.
...
- [x] `services/taxguard/embeddingService.ts` — the injectable `EmbeddingsClient` seam over Voyage AI or Gemini (env-selected, 2026-09-22) via `fetch`, no new dependency
