...
- `items` is the one product master, shown as **Products & Services**, with `item_type` `SERVICE` / `NON_INVENTORY` (created in LedgerCore) and `INVENTORY` (created in StockLedger; `FIXED_ASSET` reserved). Services are created here; stock is created in StockLedger and appears here automatically.
...
- The seam: `invoiceService`/`billService` call `services/stock/documentStockService.ts` (the only StockLedger module LedgerCore imports) on their own transaction client, then build their own journal from the values it returns. Lock order: document row → stock balances (sorted) → invoice number → journal.
...
- **Not built:** lot/serial items and fixed assets on document lines (422), consumption issues, credit/debit-note restocking, inventory as a control account, depreciation.
