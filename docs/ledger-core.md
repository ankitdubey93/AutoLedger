...
- [x] `walkthrough/` at the repo root — a committed, three-month hand-enterable accounting scenario (vendor bills, customer invoices, three bank statements in three different formats) with a computed answer key, verified end to end against these real services in `walkthrough.test.ts`. Run `npm run walkthrough` to regenerate it; see `walkthrough/README.md` and `walkthrough/TUTORIAL.md`
