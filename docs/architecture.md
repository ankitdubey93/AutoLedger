...
**Phase 13 (2026-09-11)** added, on the server: `db/migrations/{035_forecaster_plans,036_forecaster_drivers,037_forecaster_headcount,038_forecaster_forecast_lines,039_forecaster_budgets}.sql`; `types/forecaster.ts` (`ForecasterPlanStatus`/`FORECASTER_PLAN_TRANSITIONS`, `ForecasterDriverKind`, `ForecasterLineKind`, `ForecasterBudgetStatus`/`FORECASTER_BUDGET_TRANSITIONS`, `ForecasterBudgetLineSource`); `utils/forecasterBuild.ts` (the pure forecast build engine — no database import, unit-tested without Postgres, mirroring `utils/fpaProjection.ts`'s own posture); `schemas/forecaster/{planSchema,driverSchema,headcountSchema,forecastLineSchema,budgetSchema}.ts`; `services/forecaster/{planService,driverService,headcountService,forecastLineService,forecastService,budgetService,varianceService}.ts`; `controllers/forecaster/{planController,driverController,headcountController,forecastController,budgetController}.ts`; `routes/forecaster/{index,planRoutes,driverRoutes,headcountRoutes,forecastLineRoutes,budgetRoutes,budgetLineRoutes}.ts`; and `__tests__/{forecasterBuild,forecaster/{plans,drivers,headcount,forecast,budgets,variance,forecasterConstraints}}.test.ts`. `package.json` unchanged — no new dependency. Table prefix `forecaster_`, app slug `forecaster`, mounted at `/api/v1/forecaster` — the same one-line-per-app pattern `ledger-core`/`ap-flow`/`fpa-engine` already establish in `config/apps.ts` and `routes/index.ts`.

Edited in place: `config/apps.ts` (`forecaster` → `'building'`), `routes/index.ts` (mounts `/forecaster`), `__tests__/helpers/factories.ts` (`resetTables()` truncates the six new tables).

On the client: `Pages/forecaster/{ForecasterRoutes,ForecasterSidebar,ForecasterPlansPage,NewForecasterPlanPage,ForecasterPlanDetailPage,ForecasterForecastPage,ForecasterBudgetPage,ForecasterVariancePage}.tsx`, plus three new test files (`forecasterPlans`, `forecasterForecast`, `forecasterBudget`). Edited in place: `apps/registry.ts` (`forecaster` entry), `services/fetchServices.ts` (the ForecasterPro types and wrappers).
...
**Phase 19.3 introduced the same idea at subdirectory scale.** `services/integrations/` (and its `controllers/`, `routes/`, `schemas/` counterparts) sit alongside `services/ledger-core/` and `services/ap-flow/` in shape — a named subdirectory, not a bare file — but hold shared infrastructure in exactly the sense above: an integration has no app of its own, and its dispatcher's whole job is calling *into* an app's service, never reading its tables. A single-file shared service and a shared-infrastructure subdirectory are the same layer-root category at two different sizes, chosen by how much code the concern actually needs — one file for `storageService.ts`'s job, a subdirectory once a concern grows a connection lifecycle, several destinations, and its own schema.
