...
AutoLedger is the suite name. Inside it are three portfolio applications — LedgerCore, AP-Flow, StockLedger — see [roadmap.md](roadmap.md#app-map) for what each one does. Five further apps were built and then removed from the suite — see [roadmap.md § Phase 29, as delivered](roadmap.md#phase-29-as-delivered) for which ones and why.
...
- **App layer** — everything else. Each app's routes mount at `/api/v1/<app-slug>/<module>` (see `config/apps.ts` for the slugs, e.g. `ledger-core`, `stock`). An app's own frontend pages live under `client/src/Pages/<app-slug>/`.
