...
**Applied: `001`–`053`.** Every phase's tables below are marked `— applied` except **Phase 17** (QuickBooks sync), which stays target state — deferred, unbuilt. This line was last correct at `026`; it drifted for a long stretch before this pass caught it, which is itself the reminder to re-verify it against `server/src/db/migrations/` on every schema-touching change rather than trusting it by inertia.
