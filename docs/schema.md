...
All eight apps share one database and one migration sequence. Table names disambiguate which app owns them:
...
- **Every other app prefixes its own tables** with its slug: `ap_flow_invoices`, `fpa_scenarios`, `taxguard_documents`, `unitecon_product_lines`, `boarddeck_decks`, `forecaster_budgets`, `stock_items`. An app's tables are never read by another app directly — cross-app effects go through LedgerCore's GL via `source_type` / `source_id`. UnitEcon has no `unitecon_cohorts` table — a cohort matrix is derived on every request, never stored (see Phase 14 below). StockLedger (Phase 28) does **not** post to the GL at all — every `stock_*` table stands alone with no `source_type`/`source_id` hook, the one app not yet bridged.
...
**Why `entity_id` carries no `REFERENCES`, the identical rule-16-over-rule-8 ruling `document_links.entity_id`/`journal_entries.source_id`/`ap_flow_line_items.account_id` already carry.** A platform table must not hard-wire an FK into an app's own schema — AP-Flow's `ap_flow_documents` today, TaxGuard's own tables whenever it adopts this table next. Validity is a service-layer concern (`aiUsageService.listCallsForEntity` is read-only and tolerant of a dangling id).
...
**Two documented exceptions to guardrails rule 1** live in `services/integrations/driveConnectionService.ts` (moved from `services/ap-flow/` in Phase 19.3), not the schema: the OAuth callback's state lookup (no session exists yet — the state hash *is* the credential) and the scheduler's due-folders query (ids only, every downstream call re-scopes by that row's own `org_id`).
...
**Embedding dimension is fixed at 1024** (`voyage-3.5` or `gemini-embedding-001` with `outputDimensionality: 1024`, per `TAXGUARD_EMBEDDING_PROVIDER`) in the column type itself. No column records which provider embedded a chunk — switching providers means re-adding every corpus document. Changing it is a new migration plus a full re-embed of every chunk, not a value to tune casually.
...
`065_stock_setup.sql`, `066_stock_items.sql`, `067_stock_movements.sql` — 12 tables across three migrations. Full spec: [stock.md](stock.md).
...
See [api.md](api.md#stockledger--apiv1stock--phase-28) for the routes and [stock.md](stock.md) for the industry profiles, code-pattern grammar, attribute rules, tracking/valuation rulings, the serial FSM, the lock-order rule and the QR payload.
