...
| `ANTHROPIC_API_KEY` | no — defaults `''` | Phase 10 — AP-Flow's vision extraction. The server and worker both boot without it; `extractionService` throws `503` only when a real call is attempted with no key configured. Every test stubs the client, so the suite needs no key at all |
| `CAPTURE_AI_PROVIDER` | no — defaults `anthropic` | Phase 19 (named `AP_FLOW_AI_PROVIDER` until Phase 33 — rename it in an existing `.env`) — which of `anthropic` \| `gemini` the bill inbox's extraction and classification use. Only the selected provider's key needs to be set |
| `GEMINI_API_KEY` | no — defaults `''` | Phase 19 — AP-Flow's second extraction provider, called over `fetch` (no SDK — rule 14). Needed only when `CAPTURE_AI_PROVIDER=gemini`. Note: on Google's free tier, prompt content may be used to improve Google's products — AP-Flow only ever sends already-redacted pages, but confirm which tier your key is on |
| `CAPTURE_GEMINI_MODEL` | no — defaults `gemini-3.6-flash` | Phase 19. Pinned deliberately, not the floating `gemini-flash-latest` alias — a model must not change under a live ledger without a diff. Retiring one is expected maintenance: the original default, `gemini-2.5-flash`, started returning `404` ("no longer available to new users") on 2026-09-14 even though it still appeared in Google's `ListModels`; only `generateContent` had actually stopped serving it |
...
| `GOOGLE_OAUTH_REDIRECT_URI` | no — defaults `http://localhost:5000/api/v1/integrations/drive/oauth/callback` | Phase 19.2, path **changed in 19.3** when the route moved off `/api/v1/capture/drive`. Must exactly match an authorized redirect URI on the Google Cloud OAuth client. A `.env` still pinning the pre-19.3 path (`/api/v1/ap-flow/drive/oauth/callback`) **stops working in Phase 33** — that alias was removed with the `/ap-flow` prefix; point the URI (and the Google Cloud OAuth client) at the default. After a successful or failed connect the browser lands on `/settings/connections?drive=…`. |
...
**The Postgres image is `pgvector/pgvector:pg16`, not stock `postgres:16`.** It is `postgres:16` plus the `vector` extension, built `FROM postgres:16`, so the existing `postgres-data` volume is binary-compatible — no data is lost by the swap. An app that once needed `CREATE EXTENSION vector` for RAG retrieval was retired in Phase 29 (migration `068` drops the extension again), but migration `044` still runs on a from-scratch `npm run db:reset` and still calls `CREATE EXTENSION vector`, so the image cannot go back to stock `postgres:16` without breaking a clean migrate. See [schema.md § Phase 29](schema.md#phase-29--five-apps-retired-platform--applied).
...
**Phase 19 added no dependency, server or client.** AP-Flow's second extraction provider (Gemini) is called over the built-in `fetch` — no `@google/genai` or `googleapis` SDK, the same no-SDK call this codebase has made for every provider-over-`fetch` integration since. The multi-provider seam (`services/capture/modelClient.ts`), the bill-posting rewrite, the auto-post gate, and the direct-upload route all reuse `@anthropic-ai/sdk`, `multer`, `zod` and hand-written utilities already in the tree.
...
**Phases 12–16 each added their own dependency (`pptxgenjs` for Phase 15's `.pptx` generation; a `docker-compose.yml` image swap plus Voyage AI/`@anthropic-ai/sdk`/`pdfjs-dist` reuse for Phase 16's RAG retrieval) and no client-side package.** Both apps, and the other three built in this range (Phases 12–14, which added no dependency at all), were **removed in Phase 29** — see [roadmap.md § Phase 29, as delivered](roadmap.md#phase-29-as-delivered). **`pptxgenjs` was left in `package.json`, now unused** — Phase 29 was a deletion phase for app code only; auditing and removing now-orphaned dependencies was explicitly out of that phase's scope (rule 14 governs adding a dependency, not removing one) and is recorded there as a follow-up. The pgvector Postgres image was **not** rolled back either — see the note above.
...
**Phase 19.2 (Google Drive folder intake) added no dependency either.** No `googleapis` — OAuth 2.0 + PKCE and the Drive v3 REST surface are hand-rolled over the built-in `fetch` in `services/integrations/googleDriveClient.ts` (moved from `services/capture/` in Phase 19.3), the identical no-SDK call `modelClient.ts`'s Gemini adapter already made. AES-256-GCM (`utils/secretBox.ts`) and SHA-256/PKCE (`utils/pkce.ts`) both come from Node's built-in `node:crypto`, the same module `utils/webhookSignature.ts` already uses.
...
| Package | Layer | Why |
|---|---|---|
| `qrcode` | server | StockLedger's label generation (`services/inventory/labelService.ts`) — renders a QR payload to SVG server-side, so the physical label size and print scaling stay lossless and the payload-construction logic (the frontend base URL) never has to ship to the client. `@types/qrcode` alongside it, dev-only |
