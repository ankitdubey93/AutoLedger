...
**One deliberate scope correction against this doc's own build ladder, made before writing any code:** the ladder's "tax split into `1180` / `2140`" is wrong on its own terms — the two paragraphs above it in [ap-flow.md](ap-flow.md) already say input tax on a *purchase* is an asset (a claim against the tax authority), and `2140 GST/VAT Output Payable` is the *sales* side, which AP-Flow never touches. Posting debits only `1180`; no AP-Flow code path writes `2140`, proven in `posting.test.ts`. **A second correction:** this table's own Phase 11 row and `config/apps.ts`'s AP-Flow skills list both named "3-way matching" and "COGS tracking," neither of which appears in [ap-flow.md](ap-flow.md)'s Phase 11 ladder or acceptance criteria — the detailed spec is authoritative per this table's own note, so neither was built, and both strings are corrected below and in `config/apps.ts`.
