...
**A module without a cross-tenant isolation test is not done.** Treat a missing one as a blocking gap, not a TODO. This is per module, not per product — LedgerCore's fixture proves nothing about AP-Flow's tables, so each app's first org-scoped module needs its own instance of the fixture below.
...
## A module prefix is not a tenancy boundary

`/inventory/...` and `/capture/...` namespace routing only — nothing in the URL may leak into a query as if it scoped anything. Add one assertion alongside the forged-`org_id` case:

```ts
it('does not answer a module resource under the wrong prefix', async () => {
  // Hitting a module's route through another module's prefix (or a retired
  // pre-Phase-33 app prefix) should 404 at the router level — a route that
  // responds to the wrong prefix is a routing bug.
  const res = await request(app)
    .get('/api/v1/capture/journals')
    .set('Cookie', tokenFor(userA, orgA));
  expect(res.status).toBe(404);
});
```
