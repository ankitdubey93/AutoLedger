...
import { isInventoryItem, LineLocationSelect, ProductPicker } from '../products/ProductPicker';
import QuickAddProductDialog from '../products/QuickAddProductDialog';
import { useStockLineData } from '../products/useStockLineData';
...
function emptyLine(): DraftLine {
  return { description: '', quantity: '', unitPrice: '', expenseAccountId: '', taxRate: '', itemId: '', stockLocationId: '' };
}
...
export default function NewBillPage() {
  ...
  function applyItemRecord(index: number, item: Item) {
    updateLine(index, {
      ...
      // An INVENTORY item posts to its inventory (asset) account, not an expense one.
      expenseAccountId: (item.itemType === 'INVENTORY' ? item.assetAccountId : item.expenseAccountId) ?? '',
    ...
    try {
      const body = {
        ...
        lines: lines.map((line) => ({
          ...
          // Only an INVENTORY line receives stock; a location on any other line is a 422.
          stockLocationId:
  ...
  if (notEditable) {
    return (
      <section className="flex flex-col gap-3">
        <BackLink to="/expenses" label="Back to expenses" />
    ...
    <section className="flex flex-col gap-4">
      ...
      <form onSubmit={(e) => void handleSubmit(e)} className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          <label className="flex flex-col gap-1 text-sm flex-1 min-w-56">
            ...
            <Link to="/vendors?new=1" className="text-xs">
              + New vendor
            </Link>
          </label>
          ...
          <div className="text-sm">
            ...
            {rateError !== null && (
              <p className="status status--bad m-0">
                {rateError} — <Link to="/fx-rates">add one under Currency → Rates</Link>.
              </p>
            )}
                  ...
                  <tr key={index} className="border-t border-[var(--border)]">
                    ...
                    <td className="p-2">
                      <select
                        ...
                        disabled={isInventoryItem(items.find((i) => i.id === line.itemId))}
                        title={
                          isInventoryItem(items.find((i) => i.id === line.itemId))
                            ? 'Inventory items post to their inventory account'
                            : undefined
                        }
                        className={inputClass}
                      >
      ...
      {quickAddLine !== null && (
        <QuickAddProductDialog
          ...
          defaultAccountId={lines[quickAddLine]?.expenseAccountId ?? ''}
