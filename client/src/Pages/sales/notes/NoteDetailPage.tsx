...
export default function NoteDetailPage({ config }: { config: NoteKindConfig }) {
  ...
  async function run(action: 'issue' | 'void' | 'delete') {
    ...
    try {
      if (action === 'delete') {
        ...
        void navigate(`/${config.originalPath}/${note.originalId}`);
        return;
      }
  ...
  const backLink = <BackLink to={`/${config.path}`} label={`Back to ${config.plural.toLowerCase()}`} />;
      ...
      <header className="flex items-baseline justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-[var(--muted)] m-0 mt-1 flex items-center gap-2">
            ...
            <Link to={`/${config.originalPath}/${note.originalId}`}>{note.originalLabel}</Link>
          </p>
        </div>
        <div className="flex items-center gap-2">
          {note.status === 'DRAFT' && (
            <>
              <Link to={`/${config.path}/${note.id}/edit`} className={`${buttonClass} no-underline`}>
                Edit
              </Link>
      ...
      <dl className="grid grid-cols-2 sm:grid-cols-3 gap-4 m-0">
        {note.journalEntryId !== null && (
          <div>
            ...
            <dd className="m-0">
              <Link to={`/journals/${note.journalEntryId}`}>{note.journalEntryId.slice(0, 8)}</Link>
            </dd>
          </div>
        )}
        {note.voidJournalEntryId !== null && (
          <div>
            ...
            <dd className="m-0">
              <Link to={`/journals/${note.voidJournalEntryId}`}>{note.voidJournalEntryId.slice(0, 8)}</Link>
            </dd>
          </div>
        )}
      </dl>
                ...
                {note.allocations.map((a) => (
                  <tr key={a.id} className="border-t border-[var(--border)]">
                    <td className="p-2 font-mono text-xs">
                      <Link to={`/${config.originalPath}/${a.documentId}`}>{a.documentLabel}</Link>
                    </td>
