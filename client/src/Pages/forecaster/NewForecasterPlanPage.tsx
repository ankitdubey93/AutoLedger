...
import { createForecasterPlan } from '../../services/fetchServices';
...
export default function NewForecasterPlanPage() {
  ...
  // The same rule the server enforces (migration 035's CHECK) — checked
  ...
  const actualsThroughInvalid = startsOn !== '' && actualsThrough !== '' && actualsThrough >= startsOn;
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      const res = await createForecasterPlan({
        name,
  ...
  return (
    <section className="flex flex-col gap-4 max-w-xl">
      ...
      <h2 className="text-lg font-semibold m-0">New plan</h2>
      ...
      <form onSubmit={(e) => void handleSubmit(e)} className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <label className="flex flex-col gap-1 text-sm flex-1 min-w-40">
            <span className="text-[var(--muted)]">Plan starts</span>
            <input
              type="month"
              required
              value={startsOnMonth}
