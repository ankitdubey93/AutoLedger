...
import { ClipboardCheck, FileBarChart, Presentation } from 'lucide-react';
import { useAppBasePath } from '../../apps/useAppBasePath';

/**
 * BoardDeck's own navigation, mirroring UniteconSidebar's `{to, label,
 * icon, end}` + `NavLink` idiom.
 */
const NAV_ITEMS = [
  { to: 'close', label: 'Close', icon: ClipboardCheck, end: false },
  { to: 'bva', label: 'Budget vs Actual', icon: FileBarChart, end: false },
  { to: 'decks', label: 'Decks', icon: Presentation, end: false },
] as const;

export default function BoardDeckSidebar() {
  const base = useAppBasePath();
  return (
    <nav
      aria-label="BoardDeck Automator"
