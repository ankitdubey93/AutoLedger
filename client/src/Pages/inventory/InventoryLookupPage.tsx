...
/**
 * A scanned QR's payload — see labelService.ts's header on the server. Labels
 * printed before Phase 33 encode `/app/stock/scan/...`; newer ones encode
 * `/inventory/scan/...`. Both stay valid for as long as old labels exist.
 */
const SCAN_URL_PATTERN = /\/(?:app\/stock|inventory)\/scan\/(item|lot|serial|location)\/([0-9a-f-]{36})\s*$/i;

/**
 * Inventory scan-and-lookup: one input that works equally well typed
