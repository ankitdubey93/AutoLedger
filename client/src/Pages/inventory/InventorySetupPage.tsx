...
/**
 * Inventory setup: choose an industry, preview what it seeds, apply it.
 * Applying is additive (setupService.applyIndustryProfile never removes or
 * overwrites), so an already-configured org gets a secondary "add another
 * template" flow that reuses the same three steps and the same endpoint.
 *
 * `?welcome=1` is the last, optional step of the setup wizard (Phase 33):
 * the page says so and offers to skip it. Inventory is optional, so skipping
 * it records nothing.
 */
export default function InventorySetupPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchStockSettings(controller.signal), fetchStockProfiles(controller.signal)])
      ...
      .catch((err: unknown) => {
        ...
        setError(err instanceof Error ? err.message : 'Could not load inventory setup');
      });
  ...
  if (created !== null) {
    return (
      <div className="space-y-4">
        <h1 className="text-lg font-semibold text-[var(--text)]">Inventory is set up</h1>
  ...
  if (settings.configured && !addingAnother) {
    ...
    return (
      <div className="space-y-4">
        <h1 className="text-lg font-semibold text-[var(--text)]">Inventory setup</h1>
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-lg font-semibold text-[var(--text)] m-0">{welcome ? 'Do you keep stock?' : 'Inventory setup'}</h1>
          {welcome && (
            <p className="text-sm text-[var(--muted)] m-0 mt-1">
              Step 4 of 4, optional. Set up inventory now if you buy and sell goods you count, or skip it and set it up
              later from Settings.
            </p>
          )}
        </div>
        {welcome && (
          <Link to="/" className="text-sm shrink-0">
            Skip for now
          </Link>
        )}
      </header>
