...
export default function NoteFormPage({ config }: { config: NoteKindConfig }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      void navigate(`${base}/${config.path}/${saved.id}`);
    } catch (err: unknown) {
  ...
  const backTo =
    existing !== null
      ? { to: `${base}/${config.path}/${existing.id}`, label: `Back to ${config.title.toLowerCase()}` }
      : original !== null
        ? { to: `${base}/${config.originalPath}/${original.id}`, label: `Back to ${config.originalNoun}` }
        : { to: `${base}/${config.path}`, label: `Back to ${config.plural.toLowerCase()}` };

  if (loadError !== null) {
    return (
      <section className="flex flex-col gap-3">
        <BackLink to={`${base}/${config.path}`} label={`Back to ${config.plural.toLowerCase()}`} />
