...
export const getJournalEntries = async (page: number = 1, limit: number = 20) => {
  const response = await fetchWithAutoRefresh(`${API_BASE_URL}/journals?page=${page}&limit=${limit}`);
  return response.json();
};
