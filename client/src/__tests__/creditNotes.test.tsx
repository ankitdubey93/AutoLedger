...
describe('CreditNotesPage', () => {
  it('renders the list with number, against-document and unapplied amount', async () => {
    ...
    expect(number).toHaveAttribute('href', '/credit-notes/cn-2');
    expect(screen.getByRole('link', { name: 'INV-000015' })).toHaveAttribute('href', '/invoices/inv-15');
...
describe('NewCreditNotePage', () => {
  it('pre-fills lines from the invoice and defaults the account to 4800', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/credit-notes/new?invoiceId=inv-15']}>
        <Routes>
...
function renderDetail() {
  return render(
    <MemoryRouter initialEntries={['/credit-notes/cn-1']}>
      <Routes>
...
describe('CreditNoteDetailPage', () => {
  it('issue confirms before posting and shows the number', async () => {
    ...
    expect(screen.getByRole('link', { name: 'je-9' })).toHaveAttribute('href', '/journals/je-9');
