...
describe('DebitNotesPage', () => {
  it('renders the list with number, against-expense and unapplied amount', async () => {
    ...
    expect(screen.getByRole('link', { name: 'INV-IC-9120' })).toHaveAttribute('href', '/expenses/bill-12');
...
describe('NewDebitNotePage', () => {
  it("pre-fills lines from the expense and asks for the vendor's credit note number", async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/debit-notes/new?billId=bill-12']}>
        <Routes>
          <Route path="/debit-notes/new" element={<NewDebitNotePage />} />
          <Route path="/debit-notes/:noteId" element={<p>detail page</p>} />
        </Routes>
      </MemoryRouter>,
    );
...
describe('DebitNoteDetailPage', () => {
  it('apply dialog defaults the amount to min(unapplied, due) and posts it', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/debit-notes/dn-1']}>
        <Routes>
