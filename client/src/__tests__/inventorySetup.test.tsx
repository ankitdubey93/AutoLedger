...
function renderSetupPage() {
  return render(
    <MemoryRouter initialEntries={['/inventory/setup']}>
      <AuthProvider>
        <Routes>
...
describe('InventorySetupPage', () => {
  ...
  it('OWNER applies and sees created counts', async () => {
    ...
    const applyButton = await screen.findByRole('button', { name: 'Set up inventory' });
  ...
  it('VIEWER sees no apply button', async () => {
    ...
    await screen.findByRole('button', { name: /Real estate/ });
    expect(screen.queryByRole('button', { name: 'Set up inventory' })).not.toBeInTheDocument();
