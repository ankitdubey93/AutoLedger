...
describe('InvoicesPage', () => {
  ...
  it('offers Edit only on a draft', async () => {
    ...
    expect(editLinks[0]).toHaveAttribute('href', `/invoices/${draft.id}/edit`);
  });
});
...
describe('InvoiceDetailPage', () => {
  it('offers Create credit note only on an issued invoice (Phase 26)', async () => {
    ...
    expect(link).toHaveAttribute('href', '/credit-notes/new?invoiceId=inv-1');
    unmount();
  ...
  it('links to the posted journal entry', async () => {
    ...
    expect(link).toHaveAttribute('href', '/journals/entry-1');
  });
});
