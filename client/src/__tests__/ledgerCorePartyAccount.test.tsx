...
import PartyAccountPage from '../Pages/ledger-core/PartyAccountPage';
...
function renderParty(kind: 'CUSTOMER' | 'VENDOR', path: string) {
  return render(
    <MemoryRouter initialEntries={[path]}>
      <Routes>
        <Route path="/app/:appSlug/customers/:partyId" element={<PartyAccountPage kind="CUSTOMER" />} />
        <Route path="/app/:appSlug/vendors/:partyId" element={<PartyAccountPage kind={kind} />} />
      </Routes>
    </MemoryRouter>,
  );
}
...
describe('PartyAccountPage', () => {
  it('renders the balance and overdue tiles from open items', async () => {
    ...
    renderParty('CUSTOMER', '/app/ledger-core/customers/cust-1');
  ...
  it('a payment row shows "applied to INV-0001" linking to the invoice', async () => {
    ...
    expect(link).toHaveAttribute('href', '/app/ledger-core/invoices/inv-1');
  });
  ...
  it('a vendor bill row is labelled "Expense" and links to the expense page', async () => {
    ...
    renderParty('VENDOR', '/app/ledger-core/vendors/ven-1');
    ...
    expect(screen.getByRole('link', { name: 'ACME-77' })).toHaveAttribute(
      'href',
      '/app/ledger-core/expenses/bill-1',
    );
  ...
  it('a 404 renders "Customer not found"', async () => {
    ...
    renderParty('CUSTOMER', '/app/ledger-core/customers/missing');
...
function renderCustomersPage() {
  return render(
    <MemoryRouter initialEntries={['/app/ledger-core/customers']}>
      <Routes>
...
describe('CustomersPage — party account links and balances', () => {
  it('the name links to the customer account and shows the balance from AR aging', async () => {
    ...
    expect(link).toHaveAttribute('href', '/app/ledger-core/customers/cust-1');
