...
## State: Phase 3.9 done — LedgerCore gets accounts payable and payments
...
**Not built:** LedgerCore stops at the trial balance, the journal register, account ledgers, sales invoicing, and accounts payable with settlement. Next is Phase 4 (P&L, balance sheet, fiscal periods with close/lock). No expense-claim/employee-reimbursement document (there is no such thing in AutoLedger, by design), no credit notes, no vendor credits, no partial void, no PDF export, no multi-currency invoices or bills — see [docs/roadmap.md](docs/roadmap.md) and the ladder in [docs/ledger-core.md](docs/ledger-core.md). The other six apps have no routes. **No audit trail yet** (Phase 5) — make no compliance claim before it lands.
