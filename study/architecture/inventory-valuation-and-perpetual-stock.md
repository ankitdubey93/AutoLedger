...
**Introduced by:** Phase 28 — StockLedger's movement engine: receive, issue, transfer, adjust, each pricing what it moves. Extended in Phase 32 — perpetual inventory *posting to the general ledger*: receipt on a bill, COGS on an invoice, voids, and a per-document stock-to-GL reconciliation check. Extended again in Phase 35a — the per-*document* check becomes a whole-*account* one: inventory becomes a real control account, self-service reconcilable, with a value-only movement pair that moves stock's value between GL accounts without moving any stock
**Verified against:** PostgreSQL 16.15, Node 22 (the Phase 32 and 35a sections were both verified by integration tests against a real database)
...
## Phase 35a — inventory as a whole-account control account

Phase 32's reconciliation check was scoped to one *document* at a time: it proved a given bill's or invoice's movements matched that document's journal entries, but said nothing about the account as a whole — a manual journal typed straight into 1140, or a product remapped to a different asset account, could leave the account's balance disagreeing with the stock ledger with nothing to catch it. Two ledgers that are each individually correct can still disagree with each other; a **control account** is the accounting device that makes that disagreement visible and refuses the postings that would cause it silently.

**What makes an account a control account here.** Not a fixed chart code (`1140` isn't special by itself) — an account is an inventory control account when it is *some* `INVENTORY` product's own `asset_account_id`, or, only when at least one `INVENTORY` product has no override of its own, the org's resolved default inventory account. `resolveInventoryControlAccountIdsOnClient` computes this set fresh on every check, because the set itself can change: link a new product to a custom asset account and that account joins the set; unlink the last product using the default and the default falls out of it. This is the same shape Phase 25 established for AR/AP (a control account is "the account real subledger activity flows through," discovered from the data, not hand-configured) extended to a third kind of subledger — see `study/postgresql/subledger-reconciliation-and-aging.md`.

**Moving value without moving stock.** The hard case a whole-account reconciliation creates: an account's mapping can change (a product remapped, a settings default edited) *after* stock already posted value to the old account. The stock itself hasn't moved — only which GL account its value is supposed to sit on has. A conventional `RECEIPT`/`ISSUE` can't express "no quantity moved, only which account the existing value counts against," so Phase 35a adds a fifth and sixth movement type, `RECLASS_OUT`/`RECLASS_IN` — a pair, `quantity_milli = 0` on both rows, opposite-signed `value_cents`, sharing one `movement_group_id`. The critical implementation fact, not just a naming detail: **`insertMovement` skips the `stock_balances` UPDATE outright for a `quantity_milli = 0` row.** It is not "the pair's two writes happen to net to zero on the balance" (an earlier draft of this mechanism said exactly that, and it's wrong) — the balance-maintenance code path simply never runs for either row of the pair. That distinction matters for concurrency: if the balance *were* updated twice (say, −value then +value), a reader between the two updates could observe (however briefly) a mid-flight state, and worse, an interleaved second transaction locking that same balance row would serialize behind two writes instead of zero. Skipping the update entirely means a reclass takes no `stock_balances` lock at all beyond the one already required to anchor the pair to a specific `(item, location, lot)` row, and no reader ever observes a transient state, because there is no intermediate state to observe — the balance row's own version simply never changes.

**Why the true-up always moves the GL, never the stock.** When the two sides disagree by more than a reclass can explain — a manual journal posted before the guard existed, say — closing the gap is `POST /inventory/reconcile/true-up`, and it is a **journal only**: `Dr/Cr` the difference against an inventory-adjustments account, no movement written. This is not a coin-flip design choice. The stock subledger is built entirely from an append-only movement ledger — every dollar of value on it traces to a real receipt, issue, or reclass, with `db/integrity.ts` checking the cached balance against it independently. The GL, by contrast, can receive a value from *outside* that discipline: a person free-typing a manual journal, an opening-balance import row, a bank rule — anything the control-account guard didn't yet exist to stop, or a case the guard's own coverage doesn't reach. When the two disagree, the *cause* is essentially always something that touched the GL side outside the movement pipeline, so the correct fix is to make the GL agree with the movements — never to invent a movement to make the movements agree with a GL number that has no receipt/issue/reclass behind it. Manufacturing a movement to close the gap would corrupt the one side of this system that is genuinely append-only and provably reconstructible.

**Refusing the postings that cause the drift in the first place.** A control account only stays reconciled if nothing can post to it outside the mechanisms this file's integrity check understands. Four write paths now refuse a direct posting to an inventory control account with `422`: a manual journal (`journalService.assertNotControlAccountsOnClient`), the staged opening-balance importer, a free-typed line on a bill at approval time, and a debit-note line. This is enforcement, not just detection — the same "refuse it at the door, and independently verify with a query" belt-and-braces pattern the rest of this codebase uses for every other invariant.

## Where it lives in this codebase

- `server/src/utils/stockValuation.ts` — `receiptValueCents`, `outflowValueCents`, `averageUnitCostCents`
- `server/src/services/inventory/movementService.ts` — the four original movement-type handlers, plus (Phase 35a) `reclassOnClient` and the `insertMovement` branch that skips the balance UPDATE for `RECLASS_OUT`/`RECLASS_IN`
- `server/src/db/migrations/067_stock_movements.sql` — the append-only trigger, the `quantity_milli >= 0` CHECK, `stock_balances`
- `server/src/db/integrity.ts` — `checkStockBalancesMatchMovements`, (Phase 32) `checkStockMovementsReconcileWithGl` (per document), (Phase 35a) `checkInventoryAccountsReconcileWithGl` (whole account, the 7th check)
- `server/src/services/inventory/documentStockService.ts` — receive/issue/reverse for invoice and bill lines, plus (Phase 35a) `reclassItemsOnClient`/`sweepReversalOnClient`; `stockGlService.ts` — journals for manual movements; `movementService.reverseMovementsOnClient` — the void value rules
- `server/src/db/migrations/072_stock_ledger_link.sql` — provenance columns, reversal types, single-reversal index
- `server/src/db/migrations/076_stock_gl_reconciliation.sql` — the two new movement types, `stock_gl_true_ups`, the pre-link backfill
- `server/src/services/accounting/inventoryAccountingService.ts` — the one accounting-side file that orchestrates a reclass (resolves accounts, calls inventory's `documentStockService` through the existing rule-16 seam, posts the journal)
- `server/src/services/inventory/valuationService.ts` — `getValuation`, `trueUp`, `reclassMisplaced`
...
- Assuming a `RECLASS_OUT`/`RECLASS_IN` pair "nets to zero on the balance" and therefore must still run the same UPDATE twice is the wrong mental model and the wrong implementation — the correct one skips the balance write for both rows entirely, because a zero-quantity movement carries no quantity/value change for the balance to apply at all.
- An organization can fail the 7th integrity check the instant it's introduced, through no fault of anything built afterward — any pre-existing manual journal to what is now a control account, or any product remapped before the guard existed, surfaces as real drift. That's the check doing its job; the fix is a true-up or a reclass, not loosening the check.
...
A: Two checks, at two scopes. A per-document check (Phase 32) compares one document's GL-linked movement values against the net debit its journal entries post to the inventory account, including reversals. A whole-account check (Phase 35a) compares the *account's* entire movement history against the *account's* entire GL balance — because a manual journal or a remapped product can throw the two out of sync with no single misbehaving document to blame. Both are queries, both run in `npm run verify:integrity` and after every test scenario, and the whole-account one is deliberately honest that pre-existing drift will fail it until someone runs a true-up.

**Q: What's the difference between an inventory *control account* and just "the inventory account"?**
A: "The inventory account" implies one fixed chart code everyone posts to. A control account is a *behavioural* guarantee: its balance is provably, checkably equal to the sum of a real subledger's activity, and nothing is allowed to post to it except through that subledger's own mechanisms. In this system the set of control accounts is computed, not configured — it's whichever accounts real `INVENTORY` products actually use, discovered from the data. AR and accounts payable got this same treatment two phases earlier (Phase 25); inventory is the third control account in the same family, and the enforcement mechanism (refuse a direct posting, independently verify with a query) is identical across all three.

**Q: A product's inventory account gets changed in Settings. Walk through what actually happens.**
A: Inside one transaction: lock every `INVENTORY` product that has no account override of its own (the ones that will be affected by a *default* change) `FOR UPDATE`, write the new default to `ledger_settings`, then for each affected product compute how much value it's currently holding on the *old* account and where it should now sit, and write one `RECLASS_OUT`/`RECLASS_IN` pair per product moving that value, plus one journal entry (Dr the new account, Cr the old one) built from all the pairs together. Stock quantities never move — only the GL-account label on already-posted value does. If the same product is mid-approval on a bill in another transaction, lock ordering (accounts resolved before balances are locked, in both paths) is what keeps this from deadlocking rather than silently racing.

**Q: Why not just let the true-up write a compensating stock movement instead of a bare journal?**
A: Because that would treat the stock subledger — the one side of this system that is genuinely append-only and independently reconstructible from source rows — as if it, too, could need correcting from an external number. It almost never does: the movement ledger only grows from real receipts, issues, transfers, adjustments and reclasses, each with its own validation, and an integrity check independently recomputes the cached balance from it. A GL difference is therefore overwhelmingly a symptom of something that bypassed the movement pipeline on the *GL* side (a manual journal, a stale posting from before a guard existed) — so the true-up journal moves the GL to match the subledger, never the reverse. Doing it the other way would let a single bad manual journal permanently distort what should be a provably-derived stock value.
...
- "What if the reclass and a bill approval for the same product race each other?" — proven, not just argued: a repeated concurrency test alternates a product's account change with a bill approval for the same product, asserting zero `40P01` deadlocks and a clean integrity check afterward. The reason it can't deadlock is the shared lock order (see `study/postgresql/transactions-isolation-pooling.md`'s `FOR SHARE`/`FOR UPDATE` section) — every path resolves which accounts it needs *before* it locks any `stock_balances` row, so two transactions can never each be holding what the other one is waiting for.
- "Would this scale to hundreds of products remapped in one settings change?" — not without more work: today a settings-default change locks and reclasses every affected product's balances inside one transaction, which is fine at SME catalogue sizes but would hold a lot of locks for a long time on a very large one; batching that into several smaller transactions is named as later work, not built.
...
- [../postgresql/subledger-reconciliation-and-aging.md](../postgresql/subledger-reconciliation-and-aging.md)
