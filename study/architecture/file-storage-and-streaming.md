...
| A per-app attachment table (each app owns its own upload/storage code) | No shared service to design or maintain | Rejected — at the time, three apps (LedgerCore, AP-Flow, BoardDeck/TaxGuard) needed files (BoardDeck and TaxGuard AI were later removed in Phase 29; the reasoning is unaffected — LedgerCore and AP-Flow alone are still two, which is enough to want one shared service); duplicating upload, hashing, and storage across them is the same promotion `redactionService.ts` already went through for the identical reason |
