...
### An FSM that started with nothing terminal, because nothing posted yet

`ApFlowDocumentStatus` (Phase 10) shipped as the first transition table in this codebase with **no empty array anywhere** in it:

```ts
// Phase 10's original shape — superseded below, kept for the argument that follows.
...
Every other FSM in this codebase earns a terminal state — or a state whose only exit carries a real side effect — because it guards something that, once posted, rule 6 forbids editing: `VOID` and `LOCKED` are terminal because a posted document or a locked period cannot un-happen; `MATCHED`'s reverse edge carries a GL side effect because unmatching has to void a real payment. `EXTRACTED` and `FAILED` here had neither restriction, and the reason was architectural rather than incidental: **Phase 10 posted nothing to the ledger.** An extraction was a draft sitting entirely in AP-Flow's own tables — re-running it via `EXTRACTED -> PENDING` or `FAILED -> PENDING` didn't touch a financial fact anywhere, because there wasn't one yet to protect. `requestReextraction` backs this with the same `canTransitionApFlowDocument` guard every other status write in this file uses, and the *database* backs it too — `ap_flow_pages`/`ap_flow_extractions` are update-immutable by trigger, so a re-extraction physically cannot edit the old attempt's rows; it deletes them and inserts a fresh set, keeping the old attempt's only remaining trace in `audit_logs`.

This is the mirror image of the `COMPLETED -> IN_PROGRESS` lesson above, arrived at from the opposite direction: there, a label that *sounded* terminal (`COMPLETED`) wasn't, because it was a promise about the past, not the future. Here, `EXTRACTED` and `FAILED` sounded like they could plausibly be terminal too — an interviewer's first guess might well be "well, once it's extracted, it's done" — but the actual test for terminality was never "does this sound final," it was always **"does an outbound edge from here require undoing a posted financial fact?"** For `LOCKED` and `VOID`, yes. For `EXTRACTED`/`FAILED` in a phase that posted nothing at all, the question didn't even apply.

### `POSTED` (Phase 11) — the terminal state the paragraph above predicted, and a second birth state (2026-09-18)

Phase 11 added real GL posting, and with it the terminal state the reasoning above said would show up the moment posting existed. A later fix (2026-09-18, AP-Flow duplicate-capture) added a second addition of a different shape — not a new terminal state, but a second **birth** state:
...
export const AP_FLOW_DOCUMENT_TRANSITIONS = {
  ...
  FAILED:     ['PENDING'],
  POSTED:     [],                 // terminal — corrected only by reversing the journal entry it produced
  DUPLICATE:  ['PENDING'],        // a second valid starting state, never reached by an UPDATE
} as const satisfies Record<ApFlowDocumentStatus, readonly ApFlowDocumentStatus[]>;
```

Every FSM elsewhere in this file assumes a single, implicit birth state — a row is `INSERT`ed as `PENDING` (or `DRAFT`, or whatever the one starting point is), and the transition table's job is purely to police what happens *after* that. `DUPLICATE` breaks that assumption on purpose: `captureDocumentService.createApFlowDocument` decides **at INSERT time**, from a business check ("does a non-duplicate row already exist for this content?"), which of two legal starting states a new row gets. The transition table still does real work here — it's what makes `DUPLICATE -> PENDING` (a human confirming "not actually a duplicate, process it") a legal, single, reviewable edge, reusing the exact mechanism (`requestReextraction`) that already un-fails a `FAILED` document, rather than inventing a parallel "confirm" endpoint that would duplicate that logic. What the table does *not* police, and structurally cannot, is the birth-state decision itself — `canTransitionApFlowDocument(from, to)` only ever answers "can a row already in state `from` move to `to`," and a row born `DUPLICATE` was never *in* any other state first. That decision lives entirely in the service function that inserts the row, which is exactly why it has to be the *one* function every entry point funnels through (direct upload, Drive folder intake, and the two-step vault-picker registration all call `createApFlowDocument`) — a second, parallel insert path would be a second place the birth-state decision could drift out of sync with this one.
...
- `server/src/types/accounting.ts` — `INVOICE_STATUSES`, `InvoiceStatus`, `isInvoiceStatus`, `INVOICE_TRANSITIONS`, `canTransitionInvoice`
- `server/src/db/migrations/009_ledger-core_invoices.sql` — the `status` CHECK and `reject_issued_invoice_mutation()` trigger
- `server/src/services/accounting/invoiceService.ts` — `issueInvoice`/`voidInvoice` call `canTransitionInvoice` before writing, never an inline string comparison
- `server/src/__tests__/accounting/invoiceConstraints.test.ts` — proves the trigger holds via raw SQL, bypassing the service entirely (same technique as `ledgerConstraints.test.ts`)
- `server/src/types/accounting.ts` — `BILL_STATUSES`, `BillStatus`, `isBillStatus`, `BILL_TRANSITIONS`, `canTransitionBill`, the four-state extension with the `AWAITING_APPROVAL -> DRAFT` recall edge
...
- `server/src/routes/accounting/billRoutes.ts` — `requireRole('OWNER', 'ADMIN', 'ACCOUNTANT')` on submit, `requireRole('OWNER', 'ADMIN')` on approve — the segregation-of-duties gate, kept separate from the FSM itself
- `server/src/__tests__/accounting/billConstraints.test.ts` — the bill half of the raw-SQL trigger proof, including the `AWAITING_APPROVAL`-editable case
- `server/src/types/accounting.ts` — `FISCAL_PERIOD_STATUSES`, `FiscalPeriodStatus`, `FISCAL_PERIOD_TRANSITIONS`, `canTransitionFiscalPeriod` — the first FSM with a state whose entire purpose is having no outbound edge
...
- `server/src/services/accounting/fiscalPeriodService.ts` — `transition()`, the one function backing `closePeriod`/`reopenPeriod`/`lockPeriod`, all three routed through `canTransitionFiscalPeriod`
- `server/src/__tests__/accounting/fiscalPeriods.test.ts` — asserts `LOCKED -> OPEN` (reopen) is rejected with `409`, the direct proof the terminal state has no path back
- `server/src/types/accounting.ts` — `BANK_TRANSACTION_STATUSES`, `BankTransactionStatus`, `isBankTransactionStatus`, `BANK_TRANSACTION_TRANSITIONS`, `canTransitionBankTransaction` — the first FSM with a non-terminal reverse edge (`MATCHED -> UNMATCHED`) that carries a GL side effect (voiding a payment) rather than touching only its own row
...
- `server/src/services/accounting/bankMatchService.ts` — `matchTransaction`/`unmatchTransaction`/`setIgnored`, each calling `canTransitionBankTransaction` before writing; `unmatchTransaction` layers an additional `row.status !== 'MATCHED'` check on top of the table so `/unmatch` and `/unignore` — two different verbs that both land on `UNMATCHED` by the table alone — stay distinct at the route level
- `server/src/__tests__/accounting/bankMatching.test.ts` — `'unmatching voids the payment and restores the amount due'` (the GL-side-effect proof), `'matching an already-matched line is 409'`, `'unmatching an unmatched line is 409'`
...
- `server/src/types/accounting.ts` — `MIGRATION_IMPORT_STATUSES`, `MigrationImportStatus`, `MIGRATION_IMPORT_TRANSITIONS`, `canTransitionMigrationImport` — the first FSM with a backward edge (`VALIDATED -> DRAFT`) that exists purely to prevent the status from lying about the data underneath it
- `server/src/services/accounting/migrationImportService.ts` — `revalidateOnClient`, the one function that recomputes and writes the import's status after every row fix, shared by both the initial staging pass and every later `PATCH`
...
- `server/src/types/capture.ts` — `AP_FLOW_DOCUMENT_STATUSES`, `ApFlowDocumentStatus`, `AP_FLOW_DOCUMENT_TRANSITIONS`, `canTransitionApFlowDocument` — six states now: `POSTED` (Phase 11) the first terminal state this table gained, `DUPLICATE` (2026-09-18) the first second-birth-state
- `server/src/services/capture/captureDocumentService.ts` — `requestReextraction`, `markProcessing`, `savePipelineResult`, `markFailed` all call `canTransitionApFlowDocument` before writing `status`; `createApFlowDocument` is the one place the birth-state decision (`PENDING` vs `DUPLICATE`) is made, outside the transition table entirely
- `server/src/db/migrations/031_ap-flow_documents.sql` — the original four-value `status` CHECK, plus `reject_ap_flow_mutation()` (update-immutable pages/extractions, DELETE still legal — the mechanism that makes `EXTRACTED -> PENDING` safe to allow at all); `054_ap-flow_duplicate_capture.sql` and `056_ap-flow_duplicate_check_relax.sql` extend the CHECK to six values and add `duplicate_of_id`
...
**Q: When AP-Flow's document FSM first shipped it had no terminal state at all. Was that a sign the FSM was incomplete or badly modeled?**
A: No — it was a correct reflection of what the FSM was actually protecting, or at that point, not yet protecting. Every terminal or side-effect-carrying edge elsewhere in this codebase exists because walking it would otherwise let someone edit a posted financial fact, which rule 6 forbids. Phase 10's scope posted nothing to the ledger at all — an extraction was a draft sitting entirely in AP-Flow's own tables, so re-running it (`EXTRACTED -> PENDING`, `FAILED -> PENDING`) never touched anything rule 6 protects, because there was no posted fact yet to protect. The database backed this up independently: `ap_flow_pages`/`ap_flow_extractions` are update-immutable, so even a re-extraction couldn't edit a prior attempt's rows in place — it deletes and re-inserts, leaving the old attempt's only trace in `audit_logs`. No terminal state wasn't a gap; it was the correct shape for a phase with nothing yet worth making a state terminal over. Phase 11 added real GL posting and, with it, `POSTED` — the terminal state the reasoning above predicted, right on schedule.

**Q: You added `DUPLICATE` as a status a row can be born into, not one it transitions into. Why not just make it a normal `UPDATE`-reachable state like everything else in the table?**
A: Because the thing deciding "should this row be `DUPLICATE`" isn't a transition at all — it's a business check ("does a non-duplicate row for this content already exist?") made once, at the moment a brand-new row is about to be inserted. There is no prior state to transition *from*: the row doesn't exist yet. Trying to force that into the transition table would mean inventing a fictitious "not yet created" state as a `from`, which buys nothing — the table's whole value is policing edges between real, persisted states. What the table *does* still own, correctly, is the one edge that matters afterward: `DUPLICATE -> PENDING`, a human saying "not actually a duplicate, process it." That edge is real, it's between two real persisted states, and reusing `requestReextraction` (the same function that un-fails a `FAILED` document) for it means the "push it through" action isn't a new code path with its own chance to drift from the FSM's own rules.

**Q: Doesn't letting a document be born in two different states risk each entry point deciding differently, or forgetting the check entirely?**
A: That's the actual risk, yes, and it's why the check has exactly one home. `captureDocumentService.createApFlowDocument` is the single function every entry point funnels through — direct upload, Google Drive folder intake, and the two-step "register an already-uploaded vault document" route all call it, none of them re-implement the duplicate lookup themselves. That was a deliberate refactor: an earlier draft had the duplicate check living in the *upload* function instead, which meant the two-step registration route — a completely separate code path exercising the same underlying table — silently kept the old behavior (a hard `409` on a second registration) while direct upload and Drive got the new one. Centralizing the check inside `createApFlowDocument` itself, run inside the same transaction as the `INSERT`, means there is structurally only one place this decision can be made, so there's nowhere for a second entry point to diverge from it.
