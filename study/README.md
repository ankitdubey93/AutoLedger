...
| [graceful-shutdown-and-process-lifecycle.md](node-express/graceful-shutdown-and-process-lifecycle.md) | Signal dispositions and exit code 143, `server.close()` vs `closeIdleConnections()`, drain ordering, unref'd watchdog timers, PID 1, why npm swallows signals, **the worker process's own drain — waiting out an in-flight job rather than a request, and what BullMQ's lock-expiry reclaim does for a job whose process is SIGKILLed — and orchestrating three dev processes from one shell script: process groups and `set -m`, signalling a pgid vs a bare pid, `SIGTTIN` on a background process group's stdin read, and `wait -n` + `pipefail` for "one dies, tear down the rest"** |
...
| [const-assertions-and-satisfies.md](typescript/const-assertions-and-satisfies.md) | Type widening, `as const` freezing to literals, deriving a union with `(typeof X)[number]`, `satisfies` as a non-widening check, why not `zod` for a static list, `isAppSlug` as the required runtime predicate, **the same `as const satisfies` shape one level deeper (ten nested industry profiles), and a `via`-tagged FSM transition table that deliberately uses a plain `Record<Key, Value>` annotation instead — exhaustiveness over keys vs. literal-value preservation as two different guarantees, and picking the one you actually need** |
...
| [discriminated-unions-and-parsers.md](typescript/discriminated-unions-and-parsers.md) | **A parser's own return type as `{ok:true,...} \| {ok:false,error}` instead of throwing, a `CodeSegment` union as a small AST, exhaustive `switch` via a `never`-typed `default`, and why a pure calculation module never imports `ApiError`** |

### PostgreSQL

| Note | Covers |
|---|---|
| [transactions-isolation-pooling.md](postgresql/transactions-isolation-pooling.md) | Session-per-connection, why `pool.query` escapes a transaction, MVCC, all four isolation levels, lost update, `FOR UPDATE`, deadlocks, `BIGINT`-as-string, **aborted-transaction state, `ON CONFLICT DO NOTHING` vs `SAVEPOINT`, `DELETE … RETURNING` as an atomic claim, and `FOR UPDATE SKIP LOCKED` as a batch queue-claim primitive — contrasted with `DELETE … RETURNING`'s one-shot consume-and-return shape; Phase 28 — locking several rows in one transaction via deterministic key ordering as the actual deadlock fix (proven by a repeated-run concurrency test, not a single pass), upsert-then-lock for a row that may not exist yet, `SERIALIZABLE`/optimistic-versioning rejected in favor of `FOR UPDATE` on a known key set, and a `FULL JOIN` across a derived cache and its append-only source needing `COALESCE` to make a nullable key comparable** |
| [migrations-and-schema-evolution.md](postgresql/migrations-and-schema-evolution.md) | Transactional DDL and why MySQL can't, what actually proves idempotency, where `IF NOT EXISTS` doesn't exist, checksums as edit-detection, session vs transaction advisory locks, expand/contract, `ALTER TABLE` lock levels, `CREATE INDEX CONCURRENTLY`, **renaming a table under the never-edit-an-applied-migration rule with a `to_regclass`-guarded `ALTER TABLE ... RENAME`, why a rename leaves every constraint/index under its old name unless renamed explicitly, swapping a CHECK constraint's definition as `DROP CONSTRAINT IF EXISTS` + `ADD CONSTRAINT`, the `<>` XOR idiom for "exactly one of two nullable columns," `ADD CONSTRAINT`'s default full-table validation scan vs `NOT VALID` + `VALIDATE CONSTRAINT`, and a CHECK colliding with an FK's own `ON DELETE SET NULL` cascade action** |
...
| [gapless-numbering-and-counters.md](postgresql/gapless-numbering-and-counters.md) | **Why a `SEQUENCE` and `MAX(n)+1` both fail for a human-facing document number, a counter row locked by a plain `UPDATE`'s implicit row lock, allocating inside the document's own transaction so a rollback un-burns the number, `ON CONFLICT DO NOTHING` as a lazy row seed, and what "gapless" actually means for an auditor; Phase 28 — a composite-keyed counter per *rendered scope* (category × year) instead of one counter per org, and a bounded retry that skips past a hand-typed code's collision rather than failing** |
...
| [partial-unique-indexes.md](postgresql/partial-unique-indexes.md) | **A unique index scoped by a `WHERE` predicate evaluated at write time — why it closes a check-then-act race a service-level `SELECT` can't, `CHECK` vs unique index vs `EXCLUDE` as the three table-level-invariant tools and which one fits which shape, `NULL`-never-equals-`NULL` in a unique index, why a partial index only serves a query whose `WHERE` implies its predicate, and mapping a `23505` on a specific constraint name (never message-text matching) to `409`; Phase 28 — a "one default row" index and the two-statement clear-then-set write it forces, a barcode partial index as a performance-only refinement over `NULL`'s existing uniqueness behavior, and `UNIQUE NULLS NOT DISTINCT` (PG 15+) for the opposite need — treating `NULL = NULL` as a real duplicate** |
| [jsonb-user-defined-attributes.md](postgresql/jsonb-user-defined-attributes.md) | **JSONB vs EAV vs a wide fixed-column table for per-tenant custom fields, `jsonb_typeof` as the database's entire (deliberately thin) contract, a normal relational table for definitions vs JSONB for values, `jsonb_path_ops` vs the default GIN opclass, why a `NUMBER`-typed value is stored as a validated string rather than a JSON number, and `\|\|` as a shallow merge-patch** |
...
| [modular-monolith-app-namespacing.md](architecture/modular-monolith-app-namespacing.md) | One deploy vs many, why the app slug is a routing convention and not a security boundary, table-naming as the data-layer half of the same convention, monorepo/microservices rejected and why, the app↔GL integration point, **a second integration point materialized (the Drive intake dispatcher routing by folder purpose, zero SQL of its own), and the generic `(result_app, result_entity_id)` pair's real cost — a real `ON DELETE SET NULL` composite FK traded away because a platform table can't hard-wire a FK into one app's schema** — **and (Phase 33) merging the apps into one product: flattening a URL namespace as a collision analysis (and the root-mount 404-vs-401 risk), frozen provenance tags because audit history is append-only, permanent redirects for printed QR labels, a `startTransition`-vs-`<Navigate>`-effect redirect race in React Router 7, and why a replayable migration history forbids dropping a column an earlier backfill reads** |
...
| [llm-structured-extraction.md](architecture/llm-structured-extraction.md) | **Forced tool use over free-form-JSON-parsing or regex-over-prose, the API-enforced `input_schema` as a shape guarantee that is not a value-level trust guarantee, re-parsing a `tool_use` block's `unknown` input through zod as a second parse-don't-validate boundary, money crossing the boundary as decimal strings so a JSON number's float representation never touches a cents value, per-field confidence as a review-routing signal that is never treated as ground truth, a pinned third-party model id as a dependency with an expiry date, why `ListModels` listing a model proves nothing about `generateContent` serving it, and the gap between a stubbed test suite proving shape and a gated live test proving liveness — including a real incident where the live test itself had a latent bug hiding it** |
| [deterministic-demo-fixtures.md](architecture/deterministic-demo-fixtures.md) | **Seeding through the real services rather than raw SQL so triggers, FSMs and the audit trail all fire genuinely, relative-offset dates resolved against an anchor month at load time vs the absolute dates that rot, decimal-string money at a file boundary as rule 3 restated where an HTTP body would normally apply it, seeding as many transactions rather than one and the rule-5 reasoning behind it, reaching a mid-pipeline FSM state through an existing persistence seam instead of re-running an external dependency, three real cross-service bugs (a capped day-of-month parameter misused for calendar arithmetic, a matcher's own candidate query silently excluding pre-settled documents, and a period-generator that creates a whole fiscal year when only one month was asked for) that no type-checker or unit test caught — only a live end-to-end run did, and — a second fixture genre, hand-enterable rather than seeded — why a fixture a human types in must fix what a seeded one can leave loose, why its own answer key is a second thing that can be self-consistently wrong, and why only an external replay through the real product (not another self-check) can actually prove it, illustrated by the one real bug that replay caught** |
| [credit-and-debit-notes.md](architecture/credit-and-debit-notes.md) | **Correcting a posted document with a second document instead of an edit: what a credit note and a debit note are (and the GST-sense debit note that is deliberately not built), their journal entries, why a note posts at its original's frozen FX rate, settlement redefined as payments + applied notes behind one SQL definition, why applying a note posts no journal entry, unapplied credit as a negative open item, cumulative caps enforced by a `FOR UPDATE` lock plus a deferred trigger, and the behaviour-preserving refactor that made the switch safe** |
| [inventory-valuation-and-perpetual-stock.md](architecture/inventory-valuation-and-perpetual-stock.md) | **Perpetual vs periodic inventory, moving average vs FIFO vs specific identification (Ind AS 2 ¶23–25) and why an item's tracking mode decides which one applies, why LIFO is prohibited, append-only movements + a derived-and-checked balance cache, the full-outflow-takes-remaining-value rounding rule, the valuation-in-processing-order gap on a back-dated receipt, and negative stock refused in both the service and a CHECK constraint** |
| [cross-app-transactional-bridge.md](architecture/cross-app-transactional-bridge.md) | **How two apps that own different tables commit one business event atomically — a public `*OnClient` function running on the caller's transaction client, the global lock order (document → balances sorted → number counter → journal) that prevents deadlocks, why the product link lives on the dependent app's side with no FK (rule 16 over rule 8), and why an outbox/event or a second transaction was rejected** |
| [barcodes-and-qr-codes.md](architecture/barcodes-and-qr-codes.md) | **The GS1 GTIN mod-10 check digit worked through by hand, QR error-correction levels and why M was chosen, why a scanned label's payload is a bare route + UUID and never a name/price/org id, SVG over PNG for lossless print scaling, server-side generation keeping payload construction trusted, and `<img src="data:...">` over `dangerouslySetInnerHTML`** |
...
| [routing-nested-and-dynamic-segments.md](react/routing-nested-and-dynamic-segments.md) | Nested routes as a tree not a lookup table, layout routes and `<Outlet/>`, dynamic segments vs splats, `useParams` typing gap, `<Navigate>` vs `navigate()`, gating a route on fetched data rather than only auth state, the remount-by-`key` cache-invalidation trick, relative-path resolution (pathname vs pathnameBase) and why the sidebar used absolute app-scoped paths, why removing chrome from a subtree is a routing change (making a route a sibling, not a child) rather than a conditional render, what an ancestor's behavior loses when a route moves out from under it, `useSearchParams` as URL-backed, linkable filter state, the list/create/detail sibling-route split, static-beats-dynamic specificity scoring, why a dynamic segment isn't itself what makes relative links fragile, a query parameter as a one-shot seed vs a live binding — the `?copyFrom=` pre-fill's effect-plus-latch pattern, why two components must share one `canReverse` rule, and the lossless cents round-trip that seeding relies on, hard vs. soft route gates: a blocking `<Navigate>` when the guarded content is unsafe or meaningless without its precondition, vs. rendering the real content behind an advisory banner when the precondition is a choice the user is allowed to defer — **and (Phase 31) why a route-param change reconciles in place instead of remounting, a fetch re-keyed from the route param to the tenant to kill a false-invalidation loading flash, splitting a stateful hook into a fetch (`useEnabledApps`) plus a pure resolver (`resolveActiveApp`) for cheap unit testing, and the per-app-sidebar-vs-one-shared-sidebar trade-off** |
| [utility-first-css-tailwind.md](react/utility-first-css-tailwind.md) | How Tailwind scans for literals (and why dynamic class names emit nothing), v4's CSS-first `@theme` config, cascade layers and why unlayered CSS beat Preflight, the honest trade utilities make, why no Tailwind utility can ever override an unlayered rule and the scoped-unlayered-rule fix, and retiring a `:has()` selector once the component boundary it detected became an explicit routing fact instead — **and (Phase 31) a three-selector `data-theme`/`prefers-color-scheme` light/dark/system theme sharing one custom-property namespace, the synchronous inline `<script>` that prevents a flash of the wrong theme before React hydrates, `color-mix()`-derived "soft" status tokens cascading from one base colour per theme, and why `useTheme()` returns a safe default instead of throwing outside a provider** |
| [accessible-dialogs-and-focus.md](react/accessible-dialogs-and-focus.md) | Why `window.confirm` is untestable in jsdom and blocks the event loop, `role="dialog"` + `aria-modal` + `aria-labelledby`, focus-on-mount, Escape and `target === currentTarget` outside-click dismissal, native `<dialog>` vs a fully-controlled component, and disambiguating same-labelled buttons with `within()` in tests — **and (Phase 31) the WAI-ARIA menu-button pattern as a distinct contract from a dialog: `role="menu"`/`role="menuitem"`, `tabIndex={-1}` removing items from Tab order, scripted roving focus over a filtered "selectable only" index array, Escape-closes-and-returns-focus-to-trigger, and a real `requestAnimationFrame`-vs-`useEffect` focus-timing bug found by writing the test** |
...
| [schema-driven-forms-and-print-layouts.md](react/schema-driven-forms-and-print-layouts.md) | **Rendering a form's fields from a runtime data array instead of hardcoded JSX, `key` vs `id` as two unrelated identifiers, a `NUMBER` field kept a string client-side too (mirroring the server's JSONB contract), debouncing a live preview via a `useEffect` cleanup function's own `setTimeout` cancellation, and `@page`/`mm`/`break-inside: avoid` for a physically-sized, printable label sheet** |
...
| [oauth2-pkce-and-secrets-at-rest.md](security-auth/oauth2-pkce-and-secrets-at-rest.md) | **The authorization-code flow and what PKCE's `S256` challenge/verifier actually defeats, why `state` doubles as both CSRF binding and the only credential a session-less OAuth callback has, hashing `state` (compare-only) vs encrypting the verifier and refresh token (must be read back), AES-256-GCM's authentication tag and why the 12-byte IV must never repeat under a key, a `v1.` version prefix for a format that may need to change, why the master encryption key stays a plain env var rather than itself being encrypted, and why two Drive-intake tables are deliberately exempt from the append-only CDC audit trail** |
| [service-accounts-and-jwt-bearer.md](security-auth/service-accounts-and-jwt-bearer.md) | **RFC 7523's JWT-bearer grant as authenticate-as-yourself instead of three-legged consent, why it issues no refresh token at all (and is therefore structurally immune to Google's 7-day Testing-mode refresh expiry), RS256 signing with `node:crypto` and no SDK, PKCS#8 vs PKCS#1 private-key PEM formats, delegation-by-sharing vs domain-wide delegation (`sub`) and why the latter was refused, and a single-flight in-flight-promise cache proving 20 concurrent callers on a cold cache produce exactly one token request** |
...
| Discriminated unions as a parser's own return type (result unions vs throwing) | 28 (StockLedger — the code-pattern tokenizer/renderer) | ✅ |
...
| `satisfies`, `as const`, deriving a union from data | 2, 28 (nested one level deeper; contrasted against a plain `Record<Key,Value>` FSM table where exhaustiveness, not literal preservation, is what's needed) | ✅ |
...
| Topic | Phase | Status |
|---|---|---|
| Transactions, isolation, pooling | 1, 3, 7 (batch `SKIP LOCKED` claim), 28 (deterministic multi-row lock ordering, upsert-then-lock, `FULL JOIN` on a nullable key) | ✅ |
...
| **Deferred constraint triggers: enforcing a multi-row invariant at `COMMIT`** | 3 (balance check), 3.8 (invoice partial immutability), 3.9 (payment allocation completeness + cross-transaction overallocation), 26 (one invariant over two allocation sources; `CREATE OR REPLACE FUNCTION` in a new migration) | ✅ |
| Gapless(-ish) numbering: counter row + row lock vs `SEQUENCE` | 3.8 (invoice numbering), 26 (separate credit/debit note series), 28 (composite-keyed per-scope counters; skip-past-collision retry for a manually typed code) | ✅ |
| **Subledger reconciliation: derived document totals vs a GL control account balance** | 3.9 (AR/AP aging), 26 (negative open items for unapplied credit) | ✅ |
...
| Partial unique indexes: a `WHERE`-scoped uniqueness check enforced at write time | 9b (one committed opening-balance import per org), 28 (one default code scheme, a barcode index, `UNIQUE NULLS NOT DISTINCT`) | ✅ |
...
| Nested routes, layout routes, dynamic segments | 2, 9a (hard vs. soft route gates), 31 (tenant-keyed fetch vs route-param-keyed fetch) | ✅ |
...
| Schema-driven form rendering from a runtime field list; debounced live preview via `useEffect` cleanup | 28 (StockLedger — per-org custom attributes, code-pattern preview) | ✅ |
| Print-specific CSS: `@page`, physical units, `break-inside` | 28 (StockLedger — the QR label sheet) | ✅ |
| Utility-first CSS; Tailwind v4's CSS-first config | 3, 31 (CSS custom-property theming: light/dark/system, no-flash inline script) | ✅ |
| React 19 specifics (`use`, Actions, compiler) | 1 | ✅ |
| Error boundaries & suspense | 3+ | ◐ |
| Accessible confirmation dialogs: ARIA roles, focus, dismissal | 3.8 (reverse/issue/void confirmations), 31 (the menu-button pattern as a distinct contract) | ✅ |
...
| Modular monolith: app boundaries without a network boundary | 2, 33 (merging apps into one product's modules) | ✅ |
...
| Edit distance (Levenshtein DP) & confidence scoring | 6 (bank reconciliation), 6.1 (the third resolution — post a journal entry — and why it isn't interchangeable with ignore) | ✅ |
...
| Optimistic vs pessimistic concurrency control | 28 (StockLedger — deterministic-order `FOR UPDATE` on balance/serial rows, `SERIALIZABLE`/optimistic-versioning rejected and why) | ✅ |
...
| FIFO / weighted-average-cost valuation algorithms | 28 (StockLedger — moving average + specific identification built; FIFO named as the next build-ladder step, not built) | ◐ |
| Perpetual inventory posting to the GL — COGS in the sale's own entry, stock-ledger ↔ GL reconciliation, void valuation | 32 (Products & Services / StockLedger bridge) | ✅ |
| Atomic cross-module operations — `*OnClient` on the caller's transaction, global lock order, pointer on the dependent side | 32 (LedgerCore ↔ StockLedger) | ✅ |
| Barcode/QR checksums and payload-minimization design | 28 (StockLedger — GS1 GTIN mod-10, QR error correction, why a label's payload carries no business data) | ✅ |
...
| OAuth 2.0 authorization code + PKCE, and third-party secrets at rest (AES-256-GCM) | 19.2 (Drive intake, OAuth path) | ✅ |
| Service-account auth and the JWT-bearer grant (RFC 7523) | 19.3 (Drive intake, service-account path) | ✅ |
...
| Testing concurrency (two clients, one row) | 28 (StockLedger — repeated opposite-direction transfers asserting zero deadlocks) | ✅ |
