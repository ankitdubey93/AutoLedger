...
| [deterministic-demo-fixtures.md](architecture/deterministic-demo-fixtures.md) | **Seeding through the real services rather than raw SQL so triggers, FSMs and the audit trail all fire genuinely, relative-offset dates resolved against an anchor month at load time vs the absolute dates that rot, decimal-string money at a file boundary as rule 3 restated where an HTTP body would normally apply it, seeding as many transactions rather than one and the rule-5 reasoning behind it, reaching a mid-pipeline FSM state through an existing persistence seam instead of re-running an external dependency, and three real cross-service bugs (a capped day-of-month parameter misused for calendar arithmetic, a matcher's own candidate query silently excluding pre-settled documents, and a period-generator that creates a whole fiscal year when only one month was asked for) that no type-checker or unit test caught — only a live end-to-end run did** |
