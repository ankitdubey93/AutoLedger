...
**The same shared-builder discipline, and the same `EXISTS` reasoning, reappears in Phase 6's bank-line register.** `bankMatchService.buildFilters` fans out to seven independent optional filters instead of the journal register's five, and one of them — `minScore` (only lines carrying at least one suggestion above a chosen score) — is expressed as exactly the `EXISTS` shape above, correlated on the outer `bank_transactions` row rather than `journal_entries`: `EXISTS (SELECT 1 FROM bank_match_suggestions s WHERE s.bank_transaction_id = bt.id AND s.org_id = bt.org_id AND s.score >= $n)`. A `JOIN` here would have the identical fan-out problem the journal register's account filter does — a bank line can carry up to five suggestions, so joining against `bank_match_suggestions` and filtering on score would return that line's row once per qualifying suggestion, needing a `DISTINCT` to undo it. `EXISTS` again stops at the first match and never duplicates the outer row, so the same construct answers "does at least one related row satisfy this condition" correctly in both places, for two otherwise-unrelated tables.
